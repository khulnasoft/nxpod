...
mount --make-shared /var/nxpod/workspaces
...
NXPOD_CHART_VERSION=${NXPOD_CHART_VERSION:-0.5.0}


echo "DOMAIN:               $DOMAIN"
echo "BASEDOMAIN:           $BASEDOMAIN"
echo "DNSSERVER:            $DNSSERVER"
echo "NXPOD_CHART_VERSION: $NXPOD_CHART_VERSION"
...
# prepare Nxpod helm installer
NXPOD_HELM_INSTALLER_FILE=/var/lib/rancher/k3s/server/manifests/nxpod-helm-installer.yaml
...
sed 's/^/    /' /default_values.yaml >> "$NXPOD_HELM_INSTALLER_FILE"

sed -i "s/\$DOMAIN/$DOMAIN/g" "$NXPOD_HELM_INSTALLER_FILE"
sed -i "s/\$BASEDOMAIN/$BASEDOMAIN/g" "$NXPOD_HELM_INSTALLER_FILE"
sed -i "s/\$NXPOD_CHART_VERSION/$NXPOD_CHART_VERSION/g" "$NXPOD_HELM_INSTALLER_FILE"

# nxpod-helm-installer.yaml needs access to kubernetes by the public host IP.
...
installation_completed_hook() {
    while [ -z "$(kubectl get pods | grep nxpod-helm-installer | grep Completed)" ]; do sleep 10; done

    echo "Removing installer manifest ..."
    rm -f /var/lib/rancher/k3s/server/manifests/nxpod-helm-installer.yaml
}
installation_completed_hook &
