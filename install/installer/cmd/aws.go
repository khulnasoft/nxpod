...
import (
	...
	"github.com/khulnasoft/installer/pkg/aws"
...
var awsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if os.Getenv("AWS_ACCESS_KEY_ID") == "" || os.Getenv("AWS_SECRET_ACCESS_KEY") == "" {
			...
			if os.Getenv("NXPOD_INSTALLER_IN_DOCKER") != "" {
		...
		if domain == "" || domain == "ipDomain" {
			ui.Infof("\nWe won't use a custom domain for this installation, but one based on ip.nxpod-self-hosted.com.\n" +
		...
		if err != nil || ingressHostname == "" {
			// TODO(cw): produce a more helpful message, e.g. manually setting the ip.mynxpod.com domain
			...
			domain = fmt.Sprintf("%s--%s.ip.mynxpod.com", segs[0], segs[1])
			err = terraform.PersistVariable(tfvarsfn, terraform.PersistVariableOpts{
				Name: "domain",
				Sources: []terraform.VariableValueSource{
