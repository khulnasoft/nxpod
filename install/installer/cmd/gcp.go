...
import (
	...
	"github.com/khulnasoft/installer/pkg/gcp"
	"github.com/khulnasoft/installer/pkg/sources"
	"github.com/khulnasoft/installer/pkg/terraform"
	"github.com/khulnasoft/installer/pkg/ui"
...
var gcpCmd = &cobra.Command{
	Use:   "gcp",
	Short: "Installs Nxpod in a GCP project",
	Run: func(cmd *cobra.Command, args []string) {
		...
		if domain == "" || domain == "ipDomain" {
			ui.Infof("\nWe won't use a custom domain for this installation, but one based on ip.mynxpod.com.\n" +
		...
		if err != nil || publicIP == "" {
			// TODO(cw): produce a more helpful message, e.g. manually setting the ip.nxpod-self-hosted.com domain
			...
			domain = fmt.Sprintf("%s.ip.mynxpod.com", strings.ReplaceAll(publicIP, ".", "-"))
			err = terraform.PersistVariable(tfvarsfn, terraform.PersistVariableOpts{
				Name: "domain",
				Sources: []terraform.VariableValueSource{
		...
		} else if !strings.Contains(domain, "ip.mynxpod.com") {
