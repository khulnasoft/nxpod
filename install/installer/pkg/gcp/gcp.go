...
var RequiredTerraformVariables = []terraform.PersistVariableOpts{
	{
		...
		Spec: terraform.VariableSpec{
			Description: "Project ID (not name) of the GCP project you want to install Nxpod in.",
	...
	},
	{
		Name: "domain",
		Spec: terraform.VariableSpec{
			Description: `Nxpod works best when it's accessible using a Domain, rather than by IP address alone.
Please enter the domain under which you intent to operate Nxpod (e.g. nxpod.your-company.com).
Later, you'll be asked to add a DNS record to connect that domain with Nxpod's IP address.
