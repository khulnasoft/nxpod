...
> The installer is an internal tool and as such not expected to be used by those external to Nxpod.
...
- A [Kubernetes cluster configured](https://www.nxpod.khulnasoft.com/docs/self-hosted/latest)
- A [TLS certificate](#tls-certificates)

Or, [open a Nxpod workspace](https://nxpod.io/from-referrer/)

The process to install Nxpod is:
...
Releases can be downloaded from [GitHub Releases](https://github.com/khulnasoft/nxpod/releases/)
Select your desired binary, download and install

1. Download the latest release with the command:

```shell
curl -fsSLO https://github.com/khulnasoft/nxpod/releases/latest/download/nxpod-installer-linux-amd64
```
...
```shell
curl -fsSLO https://github.com/khulnasoft/nxpod/releases/latest/download/nxpod-installer-linux-amd64.sha256
```

Validate the binary against the checksum file:

```shell
echo "$(<nxpod-installer-linux-amd64.sha256)" | sha256sum --check
```

If valid, the output is:

```shell:
nxpod-installer-linux-amd64: OK
```

3. Install the binary

```shell
sudo install -o root -g root nxpod-installer-linux-amd64 /usr/local/bin/nxpod-installer
```

4. Test to ensure the version you installed is up-to-date:

```shell
nxpod-installer version
```
...
nxpod-installer validate config --config nxpod.config.yaml
```
...
```shell
# Checks that your cluster is ready to install Nxpod
nxpod-installer validate cluster --kubeconfig ~/.kube/config --config nxpod.config.yaml
```
...
After a few minutes, your Nxpod installation will be available on the
specified `domain`.

## Uninstallation

The Installer generates a ConfigMap with the metadata of every Kubernetes
object generated by the Installer. This can be retrieved to remove Nxpod
from your cluster.

```shell
kubectl get configmaps nxpod-app -o jsonpath='{.data.app\.yaml}' \
...
The Nxpod Installer is designed as a way of providing you a robust and well-tested
framework for installing Nxpod to your own infrastructure. There may be times when
...
```shell
error: error validating "nxpod.yaml": error validating data: ValidationError(StatefulSet.status): missing required field "availableReplicas" in io.k8s.api.apps.v1.StatefulSetStatus; if you choose to ignore these errors, turn validation off with --validate=false
```
...
OpenVSX proxy (see [#8529](https://github.com/khulnasoft/nxpod/issues/8529)).
...
| `kind` | Y | Installation type to run - for most users, this will be `Full` | Available options are: <ul><li>`Meta`: To install the tools that make up the front-end facing side of `Nxpod` </li><li>`Workspace`: To install the components that make up the `Nxpod Workspaces`</li><li>`Full`: To install the complete setup, i.e. both `Meta` and `Workspace`</li> |
...
Nxpod must be connected to a Git provider. This can be done via the
...
Nxpod requires certain services for it to function correctly. The Installer
...
Nxpod is compatible with any registry that implements the [Docker Registry HTTP API V2](https://docs.docker.com/registry/spec/api/)
...
To configure Nxpod to use Amazon, you will need to use the in-cluster
...
Nxpod requires an instance of MySQL 5.7 for data storage.
...
Nxpod supports the following object storage providers:
...
- `nxpod.io/workload_meta`
- `nxpod.io/workload_ide`
- `nxpod.io/workload_services`
- `nxpod.io/workload_workspace_regular`
- `nxpod.io/workload_workspace_headless`
...
`ClusterIssuer` called `nxpod-issuer` - please change this if necessary.

This certificate is called `https-certificates` - please use that in your
Nxpod installer [config](#config).
...
By default, Nxpod will be installed to the `default`
[Kubernetes namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces).
To install to a different namespace, pass a `namespace` flag to the `render`
command.

```shell
nxpod-installer render --config nxpod.config.yaml --namespace nxpod > nxpod.yaml
```
...
```shell
nxpod-installer validate cluster --kubeconfig ~/.kube/config --config nxpod.config.yaml --namespace nxpod
```

**IMPORTANT**: this does not create the namespace, so you will need to create
that separately. This is so that uninstallation of Nxpod does not remove any
Kubernetes objects, such as your TLS certificate or connection secrets.

```shell
kubectl create namespace nxpod
```
