...
RUN wget https://github.com/mikefarah/yq/releases/download/3.3.2/yq_linux_amd64 && \
    ...
    ./yq w -i helm/nxpod/values.yaml version $VERSION && \
    ./yq w -i helm/nxpod/values.yaml imagePrefix $IMAGE_PREFIX/ && \
    echo ${VERSION} > version && \
    rm yq

COPY install-gcp-terraform--lib terraform/gcp
COPY install-aws-terraform--lib terraform/aws
RUN echo "chart_location = \"../helm/nxpod\"" >> installer.auto.tfvars && \
...
FROM alpine
ENV NXPOD_INSTALLER_IN_DOCKER="true"
