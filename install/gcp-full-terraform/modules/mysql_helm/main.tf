...
#   metadata {
#     name      = "${var.database.name}-${random_id.nxpod_database.hex}-mysql"
#     namespace = var.namespace
#   }

#   data = {
#     host          = "${var.database.name}-${random_id.nxpod_database.hex}-mysql"
#     root-password = random_password.nxpod_database_root.result
...
resource "helm_release" "mysql" {
  ...
  set {
    ...
    value = random_password.nxpod_database_root.result
  }
  ...
  set {
    name  = "mysqlPassword"
    value = random_password.nxpod_database_user.result
  }
    ...
    template {
      ...
      spec {
        container {
          ...
          env {
            name  = "MYSQL_HOST"
            value = "${var.database.name}-${random_id.nxpod_database.hex}-mysql"
          }
          env {
            name  = "MYSQL_USER"
            value = "root"
...
data "template_file" "nxpod_values_database" {
  ...
  vars = {
    host     = "${var.database.name}-${random_id.nxpod_database.hex}-mysql"
    password = random_password.nxpod_database_root.result
  }
}
