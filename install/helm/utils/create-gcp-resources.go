...
func main() {
	...
	fmt.Println("This is the GCP project setup script for Nxpod.")
...
func createVPCNetwork() error {
	out, err := run("gcloud", "compute", "networks", "create", "nxpod-vpc", "--bgp-routing-mode=regional", "--subnet-mode=auto")
...
func createServiceAccounts() error {
	...
	for _, sa := range serviceAccounts {
		...
		for _, role := range roles {
			out, err := run("gcloud", "projects", "add-iam-policy-binding", projectID, "--member=serviceAccount:nxpod-nodes-meta@"+projectID+".iam.gserviceaccount.com", "--role="+role)
			if err != nil {
				return fmt.Errorf("%v\n%s", err, string(out))
			}
		}
...
func createCluster() error {
	...
	_, err = run("gcloud", buildArgs([]string{"beta", "container", "clusters", "create", "nxpod-cluster"}, metapoolArgs)...)
	if err != nil && !isAlreadyExistsErr(err) {
		return err
	}
...
func setupBucketStorage() error {
	_, err := run("gcloud", "iam", "service-accounts", "create", "nxpod-workspace-syncer")
	if err != nil && !isAlreadyExistsErr(err) {
		return err
	}
	_, err = run("gcloud", "projects", "add-iam-policy-binding", projectID, "--member=serviceAccount:nxpod-workspace-syncer@"+projectID+".iam.gserviceaccount.com", "--role=roles/storage.admin")
	if err != nil {
		return err
	}
	_, err = run("gcloud", "projects", "add-iam-policy-binding", projectID, "--member=serviceAccount:nxpod-workspace-syncer@"+projectID+".iam.gserviceaccount.com", "--role=roles/storage.objectAdmin")
	if err != nil {
		return err
	}
	_, err = run("gcloud", "projects", "add-iam-policy-binding", projectID, "--member=serviceAccount:nxpod-workspace-syncer@"+projectID+".iam.gserviceaccount.com", "--role=roles/storage.objectViewer")
	if err != nil {
		return err
	}
	_, err = run("gcloud", "iam", "service-accounts", "keys", "create", "secrets/nxpod-workspace-syncer-key.json", "--iam-account=nxpod-workspace-syncer@"+projectID+".iam.gserviceaccount.com")
	if err != nil {
		return err
	}
...
func setupContainerRegistry() error {
	_, err := run("gcloud", "iam", "service-accounts", "create", "nxpod-registry-full")
	if err != nil && !isAlreadyExistsErr(err) {
		return err
	}

	_, err = run("gcloud", "projects", "add-iam-policy-binding", projectID, "--member=serviceAccount:nxpod-registry-full@"+projectID+".iam.gserviceaccount.com", "--role=roles/storage.admin")
	if err != nil {
		return err
	}
	_, err = run("gcloud", "iam", "service-accounts", "keys", "create", "secrets/nxpod-registry-full-key.json", "--iam-account=nxpod-registry-full@"+projectID+".iam.gserviceaccount.com")
	if err != nil {
		return err
	}

	sakey, err := ioutil.ReadFile(filepath.Join(cwd, "secrets", "nxpod-registry-full-key.json"))
	if err != nil {
		return err
	}
...
func createDatabase() error {
	...
	_, err = run("gcloud", "iam", "service-accounts", "create", "nxpod-cloudsql-client")
	if err != nil && !isAlreadyExistsErr(err) {
		return err
	}
	_, err = run("gcloud", "projects", "add-iam-policy-binding", projectID, "--member=serviceAccount:nxpod-cloudsql-client@"+projectID+".iam.gserviceaccount.com", "--role=roles/cloudsql.client")
	if err != nil {
		return err
	}
	_, err = run("gcloud", "iam", "service-accounts", "keys", "create", "secrets/nxpod-cloudsql-client-key.json", "--iam-account=nxpod-cloudsql-client@"+projectID+".iam.gserviceaccount.com")
	if err != nil {
		return err
	}
...
func initializeDatabase() error {
	...
	dbNxpodPassword = generateRandomPassword()
	fmt.Printf("  nxpod database user password: %s\n", dbNxpodPassword)

	// create DB init script
	initScript := []byte(`
set @nxpodDbPassword = "` + dbNxpodPassword + `";

source database/01-create-user.sql
source database/02-create-and-init-sessions-db.sql
source database/03-recreate-nxpod-db.sql

ALTER USER "nxpod"@"%" IDENTIFIED BY '` + dbNxpodPassword + `';
FLUSH PRIVILEGES;
    `)

	// start cloudSqlProxy
	cloudSQLProxyCmd := runC(cloudSQLProxy, "-instances="+projectID+":"+region+":"+dbName+"=tcp:0.0.0.0:3306", "-credential_file=secrets/nxpod-cloudsql-client-key.json")
  ...
  components:
    db:
	  gcloudSqlProxy:
		...
		credentials: secrets/nxpod-cloudsql-client-key.json
...
func setupHelm() error {
	...
	_, err = run("gcloud", "container", "clusters", "get-credentials", "--region", region, "nxpod-cluster")
	if err != nil {
		return err
	}
