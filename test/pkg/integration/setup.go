...
func Setup(ctx context.Context) (string, string, env.Environment, bool, string, bool) {
	...
	testenv.Setup(
		waitOnNxpodRunning(e.Namespace(), waitNxpodReady),
	)
...
func waitOnNxpodRunning(namespace string, waitTimeout time.Duration) env.Func {
	klog.V(2).Info("Checking status of Nxpod components...")
	return func(ctx context.Context, cfg *envconf.Config) (context.Context, error) {
		...
		err := wait.PollImmediate(1*time.Second, waitTimeout, func() (bool, error) {
			...
			klog.V(2).Info("All Nxpod components are running...")
			return true, nil
		})
		if err != nil {
			return ctx, nil
...
func logNxpodStatus(t *testing.T, client klient.Client, namespace string) {
	...
	tw.Flush()
	t.Logf("Nxpod components status:\n" + buf.String())
}
