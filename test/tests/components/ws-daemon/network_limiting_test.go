...
import (
	...
	daemon "github.com/khulnasoft/nxpod/test/pkg/agent/daemon/api"
...
func TestNetworkLimiting(t *testing.T) {
	...
	f := features.New("network limiting").
		...
		Assess("verify if network limiting works fine", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			ws, stopWs, err := integration.LaunchWorkspaceFromContextURL(t, ctx, "https://github.com/khulnasoft/empty", username, api, integration.WithNxpodUser(username))
			if err != nil {
				t.Fatal(err)
			}
			t.Cleanup(func() {
