...
func testWithoutGithubAction(ctx context.Context, gatewayLink, nxpodAccessToken, secretEndpoint string, useLatest bool) {
	...
	if localDebug {
		fmt.Printf("Exec command below to run UI tests:\n\nexport DISPLAY=:0\nexport GATEWAY_LINK=\"%s\"\nexport NXPOD_TEST_ACCESSTOKEN=\"%s\"\nexport WS_ENDPOINT=%s\nleeway run %s -Dversion=integration-test -DpublishToJBMarketplace=false", gatewayLink, nxpodAccessToken, secretEndpoint, scriptName)
		os.Exit(1)
	}
	cmdEnv := os.Environ()
	cmdEnv = append(cmdEnv, "GATEWAY_LINK="+gatewayLink)
	cmdEnv = append(cmdEnv, "NXPOD_TEST_ACCESSTOKEN="+nxpodAccessToken)
