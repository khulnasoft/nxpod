...
import (
	...
	log "github.com/khulnasoft/nxpod/common-go/log"
	"github.com/khulnasoft/nxpod/image-builder/bob/pkg/builder"
)
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			if errors.Is(err, builder.DockerfilePathNotExists) {
				...
				err = os.WriteFile("/workspace/.nxpod/bob.log", []byte("could not find Dockerfile at \""+dockerfilePath+"\". Please double-check the value specified in image.file in .nxpod.yml"), 0644)
				if err != nil {
					log.WithError(err).Error("cannot write init message to /workspace/.nxpod/bob.log")
				}
			}
		...
		if err != nil {
			...
			err := os.WriteFile("/workspace/.nxpod/bob.log", []byte(err.Error()), 0644)
			if err != nil {
				log.WithError(err).Error("cannot write error to /workspace/.nxpod/bob.log")
			}
