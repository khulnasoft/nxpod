...
abstract class AbstractNxpodPortForwardingService : NxpodPortForwardingService {
    companion object {
        const val FORWARDED_PORT_LABEL = "ForwardedByNxpod"
        const val EXPOSED_PORT_LABEL = "ExposedByNxpod"
    }
    ...
    private fun start() {
        ...
        if (isLocalPortForwardingDisabled()) {
            thisLogger().warn("nxpod: Local port forwarding is disabled.")
        }
    ...
    private fun observePortsList(): CompletableFuture<Void> {
        ...
        val statusServiceStub = StatusServiceGrpc.newStub(NxpodManager.supervisorChannel)
    ...
    private fun isLocalPortForwardingDisabled(): Boolean {
        return System.getenv("NXPOD_DISABLE_JETBRAINS_LOCAL_PORT_FORWARDING")?.toBoolean() ?: false
    }
