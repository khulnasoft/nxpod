...
# prepare build
component="nxpod-remote-$qualifier-$(date +%F_T"%H-%M-%S")"
...
scp -F "$sshConfig" -r "$tarDir/build/nxpod-remote" "$workspaceId":"$uploadDest"

# link
link="/ide-desktop/backend/plugins/nxpod-remote"
