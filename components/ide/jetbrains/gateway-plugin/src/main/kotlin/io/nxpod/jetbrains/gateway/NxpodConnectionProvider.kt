...
class NxpodConnectionProvider : GatewayConnectionProvider {
    ...
    override suspend fun connect(
        ...
        val connectionPanel = panel {
            indent {
                row {
                    ...
                    panel {
                        row {
                            icon(NxpodIcons.Logo2x).align(AlignX.CENTER)
                        }
                        row {
                            cell(phaseMessage)
                                .bold()
        ...
        GlobalScope.launch {
            ...
            var canceledByNxpod = false

            val ownerToken = client.server.getOwnerToken(connectParams.actualWorkspaceId).await()

            if (settings.additionalHeartbeat) {
                thisLogger().info("nxpod: additional heartbeat enabled for ${connectParams.resolvedWorkspaceId}")
                    ...
                    while (isActive) {
                        ...
                        if (thinClientJob?.isActive == true) {
                            try {
                                ...
                                thisLogger().error(
                                    "nxpod: failed to send additional heartbeat for ${connectParams.resolvedWorkspaceId}",
                                    t
                                )
                            }
                        } else {
                            thisLogger().debug("nxpod: thinClient is not active, skipping additional heartbeat for ${connectParams.resolvedWorkspaceId}")
                        }
        ...
        return connectionHandleFactory.createNxpodConnectionHandle(connectionLifetime, connectionPanel, connectParams)
    }
    ...
    private suspend fun resolveCredentialsWithWebSocketTunnel(
        ...
        try {
            ...
            val sshWebSocketServer = NxpodWebSocketTunnelServer(
        ...
        if (resp.statusCode != 200) {
            if (!resp.body.isNullOrBlank() && resp.body.contains("not implemented")) {
                if (!sendHeartBeatThroughSupervisorLogOnce) {
                    thisLogger().warn("nxpod: sendHeartbeat ${connectParams.actualWorkspaceId} failed: method is not implemented in supervisor")
                    sendHeartBeatThroughSupervisorLogOnce = true
                }
                return
            }
            thisLogger().error("nxpod: sendHeartbeat ${connectParams.actualWorkspaceId} failed: ${resp.statusCode}, body: ${resp.body}")
            return
        }
        thisLogger().debug("nxpod: sendHeartbeat succeed for ${connectParams.actualWorkspaceId}")
    }
