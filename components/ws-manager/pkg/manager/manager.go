...
func New(config Configuration, clientset kubernetes.Interface, cp *layer.Provider) (*Manager, error) {
	ingressPortAllocator, err := NewIngressPortAllocator(config.IngressPortAllocator, clientset, config.Namespace, config.WorkspacePortURLTemplate, config.NxpodHostURL)
	if err != nil {
...
func (m *Manager) ControlPort(ctx context.Context, req *api.ControlPortRequest) (res *api.ControlPortResponse, err error) {
	...
	if len(spec.Ports) == 0 {
		...
		for _, p := range service.Spec.Ports {
			...
			service.Annotations[fmt.Sprintf("nxpod/port-url-%d", p.Port)] = url
		}
