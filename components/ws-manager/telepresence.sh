...
if [ "$1" == "intp" ]; then
    echo "starting delve"
    dlv debug --listen=127.0.0.1:32991 --headless --api-version=2 github.com/khulnasoft/nxpod/ws-manager -- run -v --config /tmp/c/config/config.json --kubeconfig ~/.kube/config
    exit $?
fi
