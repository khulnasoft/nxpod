...
import { AuthProviderInfo, Identity, Token, User, UserEnvVarValue } from '@nxpod/nxpod-protocol';
...
export class GenericAuthProvider implements AuthProvider {
    ...
    @inject(NxpodCookie) protected nxpodCookie: NxpodCookie;
    ...
    protected async loginCallbackHandler(err: any, user: User | undefined, authBag: AuthBag, request: express.Request, response: express.Response, logPayload: object) {
        ...
        request.login(user, err => {
            ...
            // Create Nxpod 🍪 before the redirect
            this.nxpodCookie.setCookie(response);
    ...
    protected async verify(req: express.Request, accessToken: string, refreshToken: string | undefined, tokenResponse: any, _profile: undefined, done: OAuth2Strategy.VerifyCallback) {
        ...
        try {
            ...
            let currentNxpodUser = req.user as User | undefined;

            if (!User.is(currentNxpodUser)) { // no user in sessino, thus login-flow
                ...
                currentNxpodUser = await this.userDb.findUserByIdentity(identity);
                if (!currentNxpodUser) {
                    ...
                    if (usersWithSamePrimaryEmail.length > 0) {
                        currentNxpodUser = usersWithSamePrimaryEmail[0];
                ...
                if (!currentNxpodUser) {
                    ...
                    currentNxpodUser = await this.userService.createUserForIdentity(identity);
                }

                // block new users per request
                if (blockUser) {
                    currentNxpodUser.blocked = true;
                }
            } else { // current Nxpod user known from session
                let identity = currentNxpodUser.identities.find(i => i.authId === authId);

                if (!identity) {

                    // 1) there might be another Nxpod user linked with this identity, let's associate with current user
                    ...
                    if (userWithSameIdentity) {
                        ...
                        log.info(`(${strategyName}) Moving identity to most recently used user.`, { ...defaultLogPayload, authUser, currentNxpodUser, userWithSameIdentity, identity, clientInfo });
                    }
            ...
            /*
             * At this point we have found/created a Nxpod user and the user profile/setup is fetched, let's update the link!
             */

            const existingIdentity = currentNxpodUser.identities.find(i => Identity.equals(i, identity));
            if (existingIdentity) {
                let shouldElevate = false;
                let prevScopes: string[] = [];
                try {
                    const token = await this.getCurrentToken(currentNxpodUser);
            ...
            currentNxpodUser.identities = currentNxpodUser.identities.filter(i => i.authProviderId !== authProviderId);
            currentNxpodUser.identities.push(identity);

            // update user
            currentNxpodUser.name = authUser.authName || currentNxpodUser.name;
            currentNxpodUser.avatarUrl = authUser.avatarUrl || currentNxpodUser.avatarUrl;
            ...
            await this.userDb.storeUser(currentNxpodUser),
                await this.userDb.storeSingleToken(identity, token),
                await this.updateEnvVars(currentNxpodUser, envVars),
                await this.createGhProxyIdentityOnDemand(currentNxpodUser, identity)

            done(null, currentNxpodUser);
        } catch (err) {
...
interface GenericOAuthStrategyOptions {
    scope?: string | string[];
    /**
     * This should be Nxpod's hostname.
     */
    userAgent: string;
