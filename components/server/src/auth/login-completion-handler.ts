...
export class LoginCompletionHandler {
    ...
    async complete(
        ...
        if (!this.isBaseDomain(request)) {
            // (GitHub edge case) If we got redirected here onto a sub-domain (e.g. api.nxpod.io), we need to redirect to the base domain in order to Set-Cookie properly.
            const secret = crypto
                .createHash("sha256")
        ...
        // (default case) If we got redirected here onto the base domain of the Nxpod installation, we can just issue the cookie right away.
