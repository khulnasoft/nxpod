...
import { User, NxpodClient, NxpodServer } from '@nxpod/nxpod-protocol';
...
export class EnforcementController {
    ...
    @inject(NxpodServerImpl) protected readonly _nxpodServer: NxpodServerImpl<NxpodClient, NxpodServer>;
    ...
    protected nxpodServer(user: User) {
        /*
            This initialize call is a hack. NxpodServer is intended to be accessed via Wbsocket/JsonRpc (see WebsocketConnectionManager).
            Thus, initialize() needs a NxpodClient. For the methods we use here from NxpodServer we do not need this client.
            We re-use the NxpodServer to block a user in order to have one single point where the blocking logic is.
        ...
        */
        this._nxpodServer.initialize({} as NxpodClient, undefined, user);
        return this._nxpodServer;
    }
    ...
    protected addRouteToBlockUser(router: express.Router) {
        ...
        router.post("/block-user/:userid", async (req, res, next) => {
            ...
            const targetUserID = req.params.userid;
            try {
                await this.nxpodServer(callingUser).adminBlockUser({ id: targetUserID, blocked: true });
                res.sendStatus(200);
            } catch (e) {
    ...
    protected addRouteToKillWorkspace(router: express.Router) {
        ...
        router.post("/kill-workspace/:wsid", async (req, res, next) => {
            ...
            try {
                await this.nxpodServer(callingUser).adminForceStopWorkspace(targetWsID);
