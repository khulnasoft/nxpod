...
import {
    DisposableCollection, NxpodServer, NxpodClient, UserInfo, User, Workspace, WorkspaceCreationResult,
    ...
    SetWorkspaceTimeoutResult, GetWorkspaceTimeoutResult, Configuration, PortVisibility, InstallPluginsParams, UninstallPluginParams, PermissionName, NxpodTokenType, NxpodToken, AuthProviderEntry, WorkspaceInstancePort
} from '@nxpod/nxpod-protocol';
import { LicenseValidationResult, GetLicenseInfoResult, LicenseFeature } from '@nxpod/nxpod-protocol/lib/license-protocol';
...
import { UserMessageViewsDB } from '@nxpod/nxpod-db/lib/user-message-views-db';
import { UserStorageResourcesDB } from '@nxpod/nxpod-db/lib/user-storage-resources-db';
import { Env } from '../env';
...
import { AppInstallationDB } from '@nxpod/nxpod-db/lib/app-installation-db';
import { parseRepoUrl } from '../repohost/repo-url';
import { HostContextProvider } from '../auth/host-context-provider';
import { Cancelable } from '@nxpod/nxpod-protocol/lib/util/cancelable';
...
import { StopWorkspaceRequest, StopWorkspacePolicy, DescribeWorkspaceRequest, ControlPortRequest, PortSpec, MarkActiveRequest, PortVisibility as ProtoPortVisibility } from '@nxpod/ws-manager/lib/core_pb';
import { TheiaPluginService } from '../theia-plugin/theia-plugin-service';
import { ImageBuilderClientProvider, LogsRequest } from '@nxpod/image-builder/lib';
import { URL } from 'url';
import { TokenProvider } from '../user/token-provider';
import { AuthorizationService } from '../user/authorization-service';
import { AdminGetListRequest, AdminGetListResult, AdminGetWorkspacesRequest, WorkspaceAndInstance, AdminBlockUserRequest, AdminModifyRoleOrPermissionRequest, AdminModifyPermanentWorkspaceFeatureFlagRequest } from '@nxpod/nxpod-protocol/lib/admin-protocol';
import { DBNxpodToken } from '@nxpod/nxpod-db/lib/typeorm/entity/db-nxpod-token';
import { DBUser } from '@nxpod/nxpod-db/lib/typeorm/entity/db-user';
import { AuthProviderService } from '../auth/auth-provider-service';


@injectable()
export class NxpodServerImpl<Client extends NxpodClient, Server extends NxpodServer> implements NxpodServer, Disposable {
    ...
    public async getToken(query: NxpodServer.GetTokenSearchOptions): Promise<Token | undefined> {
        await this.doUpdateUser();
    ...
    public async controlAdmission(id: string, level: "owner" | "everyone"): Promise<void> {
        throw new ResponseError(ErrorCodes.EE_FEATURE, `Workspace sharing support is implemented in Nxpod's Enterprise Edition`)
    }
    ...
    public async getWorkspaces(options: NxpodServer.GetWorkspacesOptions): Promise<WorkspaceInfo[]> {
    ...
    public async sendHeartBeat(options: NxpodServer.SendHeartBeatOptions): Promise<void> {
    ...
    public async isPrebuildAvailable(pwsid: string): Promise<boolean> {
        throw new ResponseError(ErrorCodes.EE_FEATURE, "Prebuilds are implemented in Nxpod's enterprise edition")
    }

    public async createWorkspace(options: NxpodServer.CreateWorkspaceOptions): Promise<WorkspaceCreationResult> {
    ...
    /**
     * This is an explicit extension point for allowing downstream versions of Nxpod to selectively restrict access.
    ...
    public async getUserMessages(options: NxpodServer.GetUserMessagesOptions): Promise<UserMessage[]> {
        // TODO remove after IO-split
        return [];
    }

    public async updateUserMessages(options: NxpodServer.UpdateUserMessagesOptions): Promise<void> {
    ...
    public async setWorkspaceTimeout(workspaceId: string, duration: WorkspaceTimeoutDuration): Promise<SetWorkspaceTimeoutResult> {
        throw new ResponseError(ErrorCodes.EE_FEATURE, `Custom workspace timeout is implemented in Nxpod's Enterprise Edition`);
    }
    ...
    async getUserStorageResource(options: NxpodServer.GetUserStorageResourceOptions): Promise<string> {
    ...
    async updateUserStorageResource(options: NxpodServer.UpdateUserStorageResourceOptions): Promise<void> {
    ...
    async takeSnapshot(options: NxpodServer.TakeSnapshotOptions): Promise<string> {
        throw new ResponseError(ErrorCodes.EE_FEATURE, `Snapshot support is implemented in Nxpod's Enterprise Edition`);
    }
    ...
    public async getNxpodTokens(): Promise<NxpodToken[]> {
        const user = this.checkAndBlockUser("getNxpodTokens");
        return (await this.userDB.findAllNxpodTokensOfUser(user.id)).filter(v => !v.deleted);
    }

    public async generateNewNxpodToken(options: { name?: string, type: NxpodTokenType, scopes?: [] }): Promise<string> {
        const user = this.checkAndBlockUser("generateNewNxpodToken");
    ...
    public async deleteNxpodToken(tokenHash: string): Promise<void> {
        const user = this.checkAndBlockUser("deleteNxpodToken");
        const existingTokens = await this.getNxpodTokens(); // all tokens for logged in user
        ...
        return this.userDB.deleteNxpodToken(tokenHash);
    }
    ...
    async adminGetUsers(req: AdminGetListRequest<User>): Promise<AdminGetListResult<User>> {
        throw new ResponseError(ErrorCodes.EE_FEATURE, `Admin support is implemented in Nxpod's Enterprise Edition`);
    }
    ...
    async getLicenseInfo(): Promise<GetLicenseInfoResult> {
        throw new ResponseError(ErrorCodes.EE_FEATURE, `Licensing is implemented in Nxpod's Enterprise Edition`);
    }
    ...
    async updateOwnAuthProvider({ entry }: NxpodServer.UpdateOwnAuthProviderParams): Promise<void> {
        let userId: string;
        try {
    ...
    protected redactUpdateOwnAuthProviderParams({ entry }: NxpodServer.UpdateOwnAuthProviderParams) {
    ...
    async deleteOwnAuthProvider(params: NxpodServer.DeleteOwnAuthProviderParams): Promise<void> {
        let userId: string;
        try {
