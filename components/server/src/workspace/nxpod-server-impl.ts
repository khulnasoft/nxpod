...
import { UserDB, WorkspaceDB, DBWithTracing, TracedWorkspaceDB, TeamDB, DBNxpodToken } from "@nxpod/nxpod-db/lib";
import {
    AuthProviderEntry,
    AuthProviderInfo,
    Configuration,
...
} from "@nxpod/nxpod-protocol/lib/admin-protocol";
...
} from "@nxpod/nxpod-protocol/lib/util/tracing";
import { RemoteIdentifyMessage, RemotePageMessage, RemoteTrackMessage } from "@nxpod/nxpod-protocol/lib/analytics";
import { SupportedWorkspaceClass } from "@nxpod/nxpod-protocol/lib/workspace-class";
import { StopWorkspacePolicy } from "@nxpod/ws-manager/lib/core_pb";
...
import { IDEOption, IDEOptions } from "@nxpod/nxpod-protocol/lib/ide-protocol";
import {
    PartialProject,
...
import { CostCenterJSON } from "@nxpod/nxpod-protocol/lib/usage";
...
import {
    ...
    StripeCustomer,
} from "@nxpod/usage-api/lib/usage/v1/billing.pb";
...
export type NxpodServerWithTracing = InterfaceWithTraceContext<NxpodServer>;

@injectable()
export class NxpodServerImpl implements NxpodServerWithTracing, Disposable {
    constructor(
        ...
        @inject(NxpodTokenService) private readonly nxpodTokenService: NxpodTokenService,
    ...
    private client: NxpodApiClient | undefined;
    ...
    initialize(
        client: NxpodApiClient | undefined,
    ...
    private async listenForPrebuildUpdates() {
        ...
        const disableWebsocketPrebuildUpdates = await getExperimentsClientForBackend().getValueAsync(
            ...
            false,
            {
                nxpodHost: this.config.hostUrl.url.host,
            },
        );
    ...
    private async getAccessibleProjects() {
        ...
        return runWithRequestContext(
            {
                requestKind: "nxpod-server-impl-listener",
    ...
    setClient(ctx: TraceContext, client: NxpodApiClient | undefined): void {
    ...
    public async getToken(ctx: TraceContext, query: NxpodServer.GetTokenSearchOptions): Promise<Token | undefined> {
    ...
    public async startWorkspace(
        ctx: TraceContext,
        workspaceId: string,
        options: NxpodServer.StartWorkspaceOptions,
    ...
    public async getWorkspaces(
        ctx: TraceContext,
        options: NxpodServer.GetWorkspacesOptions,
    ...
    public async sendHeartBeat(ctx: TraceContext, options: NxpodServer.SendHeartBeatOptions): Promise<void> {
        traceAPIParams(ctx, { options });
    ...
    public async createWorkspace(
        ctx: TraceContext,
        options: NxpodServer.CreateWorkspaceOptions,
    ...
    async takeSnapshot(ctx: TraceContext, options: NxpodServer.TakeSnapshotOptions): Promise<string> {
        traceAPIParams(ctx, { options });
    ...
    public async getNxpodTokens(ctx: TraceContext): Promise<NxpodToken[]> {
        const user = await this.checkAndBlockUser("getNxpodTokens");
        const nxpodTokens = await this.nxpodTokenService.getNxpodTokens(user.id, user.id);
        await Promise.all(nxpodTokens.map((tkn) => this.guardAccess({ kind: "nxpodToken", subject: tkn }, "get")));
        return nxpodTokens;
    }

    public async generateNewNxpodToken(
        ctx: TraceContext,
        options: { name?: string; type: NxpodTokenType; scopes?: string[] },
    ): Promise<string> {
        traceAPIParams(ctx, { options });

        const user = await this.checkAndBlockUser("generateNewNxpodToken");
        return this.nxpodTokenService.generateNewNxpodToken(user.id, user.id, options, (dbToken: DBNxpodToken) => {
            return this.guardAccess({ kind: "nxpodToken", subject: dbToken }, "create");
        });
    }

    public async getNxpodTokenScopes(ctx: TraceContext, tokenHash: string): Promise<string[]> {
        traceAPIParams(ctx, {}); // do not trace tokenHash

        const user = await this.checkAndBlockUser("getNxpodTokenScopes");
        const nxpodToken = await this.nxpodTokenService.findNxpodToken(user.id, user.id, tokenHash);
        if (nxpodToken) {
            await this.guardAccess({ kind: "nxpodToken", subject: nxpodToken }, "get");
        }
        return nxpodToken?.scopes ?? [];
    }

    public async deleteNxpodToken(ctx: TraceContext, tokenHash: string): Promise<void> {
        traceAPIParams(ctx, {}); // do not trace tokenHash

        const user = await this.checkAndBlockUser("deleteNxpodToken");
        return this.nxpodTokenService.deleteNxpodToken(user.id, user.id, tokenHash, (token: NxpodToken) => {
            return this.guardAccess({ kind: "nxpodToken", subject: token }, "delete");
        });
    }
    ...
    async updateOwnAuthProvider(
        ctx: TraceContext,
        { entry }: NxpodServer.UpdateOwnAuthProviderParams,
    ...
    private redactUpdateOwnAuthProviderParams({ entry }: NxpodServer.UpdateOwnAuthProviderParams) {
        const safeEntry =
            "id" in entry
    ...
    async deleteOwnAuthProvider(ctx: TraceContext, params: NxpodServer.DeleteOwnAuthProviderParams): Promise<void> {
        traceAPIParams(ctx, { params });
    ...
    async createOrgAuthProvider(
        ctx: TraceContext,
        { entry }: NxpodServer.CreateOrgAuthProviderParams,
    ): Promise<AuthProviderEntry> {
        traceAPIParams(ctx, {}); // entry contains PII
    ...
    async updateOrgAuthProvider(
        ctx: TraceContext,
        { entry }: NxpodServer.UpdateOrgAuthProviderParams,
    ): Promise<AuthProviderEntry> {
        traceAPIParams(ctx, {}); // entry contains PII
    ...
    async getOrgAuthProviders(
        ctx: TraceContext,
        params: NxpodServer.GetOrgAuthProviderParams,
    ): Promise<AuthProviderEntry[]> {
        traceAPIParams(ctx, { params });
    ...
    async deleteOrgAuthProvider(ctx: TraceContext, params: NxpodServer.DeleteOrgAuthProviderParams): Promise<void> {
        traceAPIParams(ctx, { params });
    ...
    async getOnboardingState(ctx: TraceContext): Promise<NxpodServer.OnboardingState> {
