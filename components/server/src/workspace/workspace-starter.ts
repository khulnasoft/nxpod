...
import { WorkspaceImageSource, UserEnvVar } from '@nxpod/nxpod-protocol';
import { log } from '@nxpod/nxpod-protocol/lib/util/logging';
import { TracedWorkspaceDB, DBWithTracing } from '@nxpod/nxpod-db/lib/traced-db';
import * as uuidv4 from 'uuid/v4';
import { StartWorkspaceRequest, WorkspaceMetadata, EnvironmentVariable, GitSpec, PortSpec, WorkspaceType, PortVisibility, AdmissionLevel } from "@nxpod/ws-manager/lib/core_pb";
import { HostContextProvider } from "../auth/host-context-provider";
import { MessageBusIntegration } from "./messagebus-integration";
import { StartWorkspaceSpec, WorkspaceFeatureFlag } from "@nxpod/ws-manager/lib";
import { WorkspaceInitializer, SnapshotInitializer, PrebuildInitializer, GitInitializer, CloneTargetMode, GitConfig, GitAuthMethod } from "@nxpod/content-service/lib";
import { AuthorizationService } from "../user/authorization-service";
import { Permission } from "@nxpod/nxpod-protocol/lib/permission";
import { ImageBuilderClientProvider, BuildSource, BuildSourceDockerfile, BuildSourceReference, BuildRequest, BuildRegistryAuth, BuildRegistryAuthTotal, BuildStatus, ResolveWorkspaceImageRequest, BuildRegistryAuthSelective, BuildResponse } from "@nxpod/image-builder/lib";
    ...
    protected async prepareBuildRequest(ctx: TraceContext, workspace: Workspace, imgsrc: WorkspaceImageSource, user: User, ignoreBaseImageresolvedAndRebuildBase: boolean = false): Promise<{src: BuildSource, auth: BuildRegistryAuth, disposable?: Disposable}> {
        ...
        try {
            ...
            if (workspace.baseImageNameResolved && !ignoreBaseImageresolvedAndRebuildBase) {
                ...
                // All they need access to is the base image repository, as we're building the Nxpod layer only.
        ...
        const addExtensionsToEnvvarPromise = this.theiaService.resolvePlugins(user.id, { config: workspace.config }).then(
            resolvedExtensions => {
                if (resolvedExtensions) {
                    const ev = new EnvironmentVariable();
                    ev.setName("NXPOD_RESOLVED_EXTENSIONS");
