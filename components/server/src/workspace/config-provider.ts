...
import { User, WorkspaceConfig, CommitContext, Repository, ImageConfigString, ExternalImageConfigFile, ImageConfigFile, Commit, NamedWorkspaceFeatureFlag } from "@nxpod/nxpod-protocol";
...
export class ConfigProvider {
    static readonly DEFINITELY_GP_REPO: Repository = {
        ...
        owner: 'khulnasoft',
        name: 'definitely-gp',
        cloneUrl: 'https://github.com/khulnasoft/definitely-gp'
    };
    ...
    public async fetchConfig(ctx: TraceContext, user: User, commit: CommitContext): Promise<WorkspaceConfig> {
        ...
        try {
            ...
            const definitelyGpConfig = this.fetchExternalNxpodFileContent({span}, commit.repository);
            ...
            if (!customConfigString) {
                /* We haven't found a Nxpod configuration file in the context repo - check definitely-gp.
                *
    ...
    protected async fetchExternalNxpodFileContent(ctx: TraceContext, repository: Repository): Promise<{ content: MaybeContent, basePath: string }> {
        const span = TraceContext.startSpan("fetchExternalNxpodFileContent", ctx);
        ...
        try {
            ...
            const possibleConfigs = [
                [this.fetchDefinitelyGpContent({span}, `${ownerConfigBasePath}/.nxpod.yml`), ownerConfigBasePath],
                [this.fetchDefinitelyGpContent({span}, `${ownerConfigBasePath}/.nxpod`), ownerConfigBasePath],
                [this.fetchDefinitelyGpContent({span}, `${baseConfigBasePath}/.nxpod.yml`), baseConfigBasePath],
                [this.fetchDefinitelyGpContent({span}, `${baseConfigBasePath}/.nxpod`), baseConfigBasePath]
            ]
    ...
    protected async validateConfig(config: WorkspaceConfig, user: User): Promise<void> {
        ...
        if (checkoutLocation) {
            ...
            if (this.leavesWorkspaceBase(normalizedPath)) {
                ...
                throw new Error(`Checkout location must not leave the ${POD_PATH_WORKSPACE_BASE} folder. Check your .nxpod.yml file.`);
            }
        }
        ...
        if (workspaceLocation) {
            ...
            if (this.leavesWorkspaceBase(normalizedPath)) {
                ...
                throw new Error(`Workspace location must not leave the ${POD_PATH_WORKSPACE_BASE} folder. Check your .nxpod.yml file.`);
            }
        }
    }
...
export namespace ConfigProvider {
    export const DEFAULT_IMAGE = process.env.WORKSPACE_DEFAULT_IMAGE || "nxpod/workspace-full:latest";
