...
import { ResolvePluginsParams, ResolvedPlugins, TheiaPlugin, PreparePluginUploadParams, InstallPluginsParams, UninstallPluginParams, ResolvedPluginKind } from '@nxpod/nxpod-protocol';
import { TheiaPluginDB } from "@nxpod/nxpod-db/lib/theia-plugin-db";
import { Env } from '../env';
...
const builtinExtensions: PluginIndexEntry[] = require('@nxpod/nxpod-protocol/data/builtin-theia-plugins.json');
...
export class TheiaPluginService {
    ...
    get bucketName(): string {
        ...
        return `nxpod-${hostDenominator}-plugins`;
    }
    ...
    /**
     * `preparePluginUpload` is called by the frontend via the `NxpodService` in order to prepare the following upload.
     *
    ...
    protected getPublicPluginURL(pluginEntryId: string) {
        return new NxpodHostUrl(process.env.HOST_URL)
            .with({
                pathname: '/plugins',
