...
import { toIWebSocket } from '@nxpod/nxpod-protocol/lib/messaging/node/connection';
...
import { DeletedEntryGC } from '@nxpod/nxpod-db/lib/typeorm/deleted-entry-gc';
import { PeriodicDbDeleter } from '@nxpod/nxpod-db/lib/periodic-deleter';
import { OneTimeSecretServer } from './one-time-secret-server';
import { NxpodClient, NxpodServer } from '@nxpod/nxpod-protocol';

@injectable()
export class Server<C extends NxpodClient, S extends NxpodServer> {
        ...
        this.eventEmitter.on(Server.EVENT_ON_START, (httpServer) => {
            ...
            const csrfGuard: ws.VerifyClientCallbackAsync = (info: { origin: string; secure: boolean; req: http.IncomingMessage }, callback: (res: boolean, code?: number, message?: string) => void) => {
                ...
                if (this.env.kubeStage === 'prodcopy' || this.env.kubeStage === 'staging') {
                    ...
                    // This enables debugging Theia from inside Nxpod
