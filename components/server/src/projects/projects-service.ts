...
import { DBWithTracing, ProjectDB, TracedWorkspaceDB, WebhookEventDB, WorkspaceDB } from "@nxpod/nxpod-db/lib";
import {
    Branch,
    PrebuildWithStatus,
    CreateProjectParams,
...
export class ProjectsService {
    ...
    public async getRecentWebhookEvent(
        ...
        if (repoService) {
            try {
                const webhookEnabled = await repoService.isNxpodWebhookEnabled(user, project.cloneUrl);
