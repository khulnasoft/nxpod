...
export const isAllowedWebsocketDomain = (originHeader: any, nxpodHostName: string): boolean => {
    ...
    if (originHostname === nxpodHostName) {
        return true;
    }
    if (looksLikeWorkspaceHostname(originHeader, nxpodHostName)) {
        return true
    } else {
        return false;
    }
...
const looksLikeWorkspaceHostname = (originHostname: string, nxpodHostName: string): boolean => {
    // Is prefix a valid (looking) workspace ID?
    const found = originHostname.lastIndexOf(nxpodHostName);
    if (found === -1) {
        return false;
    }
