...
@suite(timeout(10000), retries(2), skipIfEnvVarNotSet("NXPOD_TEST_TOKEN_GITHUB"))
class TestGithubContextParser {
    ...
    @test public async testTreeContext_04() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/blob/nametest/src/src/server.ts');
        expect(result).to.deep.include({
            "ref": "nametest/src",
            "refType": "branch",
            "path": "src/server.ts",
            ...
            "title": "TypeFox/nxpod-test-repo - nametest/src"
        })
    }

    @test public async testTreeContext_05() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/tree/499efbbcb50e7e6e5e2883053f72a34cd5396be3/folder1/folder2');
        expect(result).to.deep.include(
            {
                "title": "TypeFox/nxpod-test-repo - 499efbbc:folder1/folder2",
                "repository": {
                    "host": "github.com",
                    "owner": "TypeFox",
                    "name": "nxpod-test-repo",
    ...
    @test public async testCommitsContext_01() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/commits/4test');
        expect(result).to.deep.include({
            "ref": "4test",
            "refType": "branch",
            "path": "",
            ...
            "title": "TypeFox/nxpod-test-repo - 4test"
        })
    }

    @test public async testCommitContext_01() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/commit/409ac2de49a53d679989d438735f78204f441634');
        expect(result).to.deep.include({
            "ref": "",
            "refType": "revision",
            "path": "",
            ...
            "title": "TypeFox/nxpod-test-repo - Test 3"
        })
    }
    ...
    @test public async testCommitContext_02_notExistingCommit() {
        try {
            await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/commit/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
    ...
    @test public async testCommitContext_02_invalidSha() {
        try {
            await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/commit/invalid');
            // ensure that an error has been thrown
            chai.assert.fail();
        } catch (e) {
    ...
    @test public async testIssueContext_01() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/issues/42');
        expect(result).to.deep.include(
            {
    ...
    @test public async testIssueThroughPullRequestContext() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/pull/42');
        expect(result).to.deep.include(
            {
                "title": "Test issue web-extension",
                "repository": {
    ...
    @test public async testBlobContext_01() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/blob/aba298d5084a817cdde3dd1f26692bc2a216e2b9/test-comment-01.md');
        expect(result).to.deep.include(
            {
                "title": "TypeFox/nxpod-test-repo - aba298d5:test-comment-01.md",
                "repository": {
                    "host": "github.com",
                    "owner": "TypeFox",
                    "name": "nxpod-test-repo",
    ...
    @test public async testBlobContext_02() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/blob/499efbbcb50e7e6e5e2883053f72a34cd5396be3/folder1/folder2/content2');
        expect(result).to.deep.include(
            {
                "title": "TypeFox/nxpod-test-repo - 499efbbc:folder1/folder2/content2",
                "repository": {
                    "host": "github.com",
                    "owner": "TypeFox",
                    "name": "nxpod-test-repo",
    ...
    @test public async testBlobContextShort_01() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/blob/499efbbc/folder1/folder2/content2');
        expect(result).to.deep.include(
            {
                "title": "TypeFox/nxpod-test-repo - 499efbbc:folder1/folder2/content2",
                "repository": {
    ...
    @test public async testBlobContextShort_02() {
        const result = await this.parser.handle({}, this.user, 'https://github.com/TypeFox/nxpod-test-repo/blob/499ef/folder1/folder2/content2');
        expect(result).to.deep.include(
            {
                "title": "TypeFox/nxpod-test-repo - 499efbbc:folder1/folder2/content2",
                "repository": {
