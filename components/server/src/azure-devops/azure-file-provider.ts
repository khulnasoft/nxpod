...
export class AzureDevOpsFileProvider implements FileProvider {
    ...
    public async getNxpodFileContent(commit: Commit, user: User): Promise<MaybeContent> {
        ...
        const yamlVersion1 = await Promise.all([
            this.azureDevOpsApi.getFileContent(user, azOrgId, azProject, repoName, commit, ".nxpod.yml"),
            this.azureDevOpsApi.getFileContent(user, azOrgId, azProject, repoName, commit, ".nxpod"),
        ]);
