...
@suite(timeout(10000), retries(2), skip(ifEnvVarNotSet("NXPOD_TEST_TOKEN_GITLAB")))
class TestGitlabRepositoryProvider {
    ...
    @test public async testSearchRepos_noMatchAgainstWholePath() {
        const result = await this.repositoryProvider.searchRepos(this.user, "nxpod-integration-tests/test", 100);
