...
@suite(timeout(10000), retries(0), skip(ifEnvVarNotSet("NXPOD_TEST_TOKEN_BITBUCKET")))
class TestBitbucketRepositoryProvider {
    ...
    @test public async testGetRepo() {
        const result = await this.repoProvider.getRepo(this.user, "nxpod", "integration-tests");
        expect(result).to.deep.include({
            ...
            webUrl: "https://bitbucket.org/nxpod/integration-tests",
        });
    }
    ...
    @test public async testHasReadAccess_positive() {
        const result = await this.repoProvider.hasReadAccess(this.user, "nxpod", "integration-tests");
    ...
    @test public async testSearchRepos_matchesSubstring() {
        ...
        expect(result).to.deep.include({
            url: "https://bitbucket.org/nxpod-integration-tests/exampul-repo",
            name: "exampul-repo",
        });
    }
    ...
    @test public async testSearchRepos_doesNotMatchPath() {
        const result = await this.repoProvider.searchRepos(this.user, "nxpod-integration-tests/exampul-repo", 100);
