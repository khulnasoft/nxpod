...
class TestBitbucketFileProvider {
    ...
    @test public async testGetFileContents() {
        const result = await this.fileProvider.getFileContent({ repository: { owner: "nxpod", name: "sample-repository" }, revision: "5a24a0c" } as Commit, this.user, "README.md");
...
This is the readme of the second branch.`);
    ...
    @test public async testGetLastChangeRevision() {
        const result = await this.fileProvider.getLastChangeRevision({ owner: "nxpod", name: "sample-repository" } as Repository, "second-branch", this.user, "README.md");
