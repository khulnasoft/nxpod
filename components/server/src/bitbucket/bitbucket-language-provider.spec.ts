...
class TestBitbucketLanguageProvider {
    ...
    @test public async testGetLanguages() {
        const result = await this.languageProvider.getLanguages({ owner: "nxpod", name: "sample-repository" } as Repository, this.user);
        expect(result).to.deep.equal({ "markdown": 100.0 });
    }

    @test public async testGetLanguagesNonExistingRepo() {
        const result = await this.languageProvider.getLanguages({ owner: "nxpod", name: "does-not-exist" } as Repository, this.user);
