...
export class SessionHandler {
    ...
    public clearSessionCookie(res: express.Response): void {
        ...
        res.clearCookie("nxpod_hashed_user_id", {
    ...
    public setHashedUserIdCookie(req: express.Request, res: express.Response): void {
        ...
        if (
            hostname === "nxpod.io" ||
            hostname === "nxpod-staging.com" ||
            hostname.endsWith("nxpod-dev.com") ||
            hostname.endsWith("khulnasoft-dev.com")
        ) {
            const existingHashedId = req.cookies["nxpod_hashed_user_id"];
            if (!existingHashedId) {
                const hashedUserId = crypto.createHash("md5").update(user.id).digest("hex");
                const oneYearInMilliseconds = 365 * 24 * 60 * 60 * 1000;

                res.cookie("nxpod_hashed_user_id", hashedUserId, {
                    domain: `.${hostname}`,
