...
import { WorkspaceConfig, User, PrebuiltWorkspaceState, GithubAppPrebuildConfig, Disposable } from '@nxpod/nxpod-protocol';
...
import { HeadlessWorkspaceEventType, HeadlessLogEvent } from '@nxpod/nxpod-protocol/lib/headless-workspace-log';
...
<g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
    <g id="button">
        ...
        <text id="in-Nxpod" opacity="0.703985305" font-family="Helvetica" font-size="140" font-weight="normal" fill="#FFFFFF">
            <tspan x="850" y="210">in Nxpod</tspan>
        </text>
    ...
    protected async handlePullRequest(ctx: Context): Promise<void> {
        ...
        try {
            const user = await this.findUserForInstallation(ctx);
            if (!user) {
                log.warn("Did not find user for installation. Someone's Nxpod experience may be broken.", { repo: ctx.repo() });
                return;
            }
