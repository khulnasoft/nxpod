...
RUN echo "#!/bin/bash"                                                            >> /app/migrate.sh && \
    echo "yarn --cwd /app/node_modules/@nxpod/nxpod-db run wait-for-db"        >> /app/migrate.sh && \
    echo "yarn --cwd /app/node_modules/@nxpod/nxpod-db typeorm migrations:run" >> /app/migrate.sh && \
