...
import { Identity, User, IdentityLookup, Token, TokenEntry, UserEnvVar, NxpodTokenType, NxpodToken } from "@nxpod/nxpod-protocol";
...
export class TypeORMUserDBImpl implements UserDB {
    ...
    protected async getNxpodTokenRepo(): Promise<Repository<DBNxpodToken>> {
        return (await this.getEntityManager()).getRepository<DBNxpodToken>(DBNxpodToken);
    }
    ...
    public async findUserByNxpodToken(tokenHash: string, tokenType?: NxpodTokenType): Promise<User | undefined> {
        const repo = await this.getNxpodTokenRepo();
        const qBuilder = repo.createQueryBuilder('nxpodToken')
            .leftJoinAndSelect("nxpodToken.user", "user");
        if (!!tokenType) {
            qBuilder.where("nxpodToken.tokenHash = :tokenHash AND nxpodToken.type = :tokenType", { tokenHash, tokenType });
        } else {
            qBuilder.where("nxpodToken.tokenHash = :tokenHash", { tokenHash });
        }
        qBuilder.andWhere("nxpodToken.deleted <> TRUE AND user.markedDeleted <> TRUE AND user.blocked <> TRUE");
    ...
    public async storeNxpodToken(token: NxpodToken & { user: DBUser }): Promise<void> {
        const repo = await this.getNxpodTokenRepo();
