...
export class TypeORMUserDBImpl extends TransactionalDBImpl<UserDB> implements UserDB {
    ...
    private async getNxpodTokenRepo(): Promise<Repository<DBNxpodToken>> {
    ...
    public async findUserByNxpodToken(
        tokenHash: string,
    ...
    public async storeNxpodToken(token: NxpodToken): Promise<void> {
        const repo = await this.getNxpodTokenRepo();
    ...
    public async deleteNxpodTokensNamedLike(userId: string, namePattern: string): Promise<void> {
        const repo = await this.getNxpodTokenRepo();
    ...
    async issueRefreshToken(accessToken: OAuthToken): Promise<OAuthToken> {
        // NOTE(rl): this exists for the OAuth server code - Nxpod tokens are non-refreshable (atm)
    ...
    async persist(accessToken: OAuthToken): Promise<void> {
        ...
        if (userAndToken) {
            ...
            dbToken = userAndToken.token as NxpodToken & { user: DBUser };
            const repo = await this.getNxpodTokenRepo();
            ...
            return this.storeNxpodToken(dbToken);
        }
    }
    async revoke(accessTokenToken: OAuthToken): Promise<void> {
        ...
        await this.deleteNxpodToken(tokenHash);
    }
