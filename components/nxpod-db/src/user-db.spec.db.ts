...
import { NxpodTokenType, Identity, Workspace, WorkspaceInstance } from "@nxpod/nxpod-protocol";
...
class UserDBSpec {
    ...
    public async findTokenAndOwner() {
        ...
        const result = await this.db.findUserByNxpodToken("tokenhash");
    ...
    @test(timeout(10000))
    public async findNxpodTokenOfUser() {
        let user1 = await this.db.newUser();
        user1.name = "ABC";
        user1.identities.push(TestData.ID1);
        user1.identities.push(TestData.ID2);
        ...
        await this.db.storeNxpodToken(token);
        const result = await this.db.findNxpodTokensOfUser(user1.id, token.tokenHash);
        expect(result).to.not.be.undefined;
    ...
    @test(timeout(10000))
    public async findAllNxpodTokensOfUser() {
        let user1 = await this.db.newUser();
        user1.name = "ABC";
        user1.identities.push(TestData.ID1);
        user1.identities.push(TestData.ID2);
        ...
        await this.db.storeNxpodToken(token2);
        const result = await this.db.findAllNxpodTokensOfUser(user1.id);
        expect(result).to.not.be.undefined;
