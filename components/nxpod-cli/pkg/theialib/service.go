...
func NewServiceFromEnv() (*HTTPTheiaService, error) {
	theiaPort := os.Getenv("NXPOD_THEIA_PORT")
	if theiaPort == "" {
		theiaPort = "23000"
	}
	url := fmt.Sprintf("http://localhost:%s/nxpod/cli", theiaPort)

	apiToken := os.Getenv("NXPOD_CLI_APITOKEN")
	if apiToken == "" {
		return nil, fmt.Errorf("No NXPOD_CLI_APITOKEN environment variable set")
	}
...
// SetEnvVar sets a nxpod environment variable
...
// GetEnvVars returns the list of Nxpod env vars for this workspace
