...
import (
	...
	supervisorapi "github.com/khulnasoft/nxpod/supervisor/api"
)
...
func connectToServer(ctx context.Context, options *connectToServerOptions) (*connectToServerResult, error) {
	...
	return &connectToServerResult{repositoryPattern, wsinfo, client, wsinfo.NxpodHost}, nil
}
				...
				if ferr, ok := err.(*jsonrpc2.Error); ok && ferr.Code == http.StatusForbidden && setEnvScope == envScopeUser {
					...
					if ferr, ok := err.(*jsonrpc2.Error); ok && ferr.Code == http.StatusForbidden {
						...
						return fmt.Errorf(""+
							...
							"Please create the var manually under %s/user/variables using Name=%s, Scope=*/**, Value=foobar", v.Name, result.nxpodHost, v.Name)
					}
				} else {
					return err
				}
