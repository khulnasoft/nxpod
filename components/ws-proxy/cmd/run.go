...
var runCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case HostBasedIngress:
			...
			go proxy.NewWorkspaceProxy(addr, cfg.Proxy, proxy.HostBasedRouter(cfg.Ingress.HostBasedIngress.Header, cfg.Proxy.NxpodInstallation.WorkspaceHostSuffix), workspaceInfoProvider).MustServe()
		...
		case PathAndHostIngress:
			...
			go proxy.NewWorkspaceProxy(addr, cfg.Proxy, proxy.PathAndHostRouter(cfg.Ingress.PathAndHostIngress.TrimPrefix, cfg.Ingress.PathAndHostIngress.Header, cfg.Proxy.NxpodInstallation.WorkspaceHostSuffix), workspaceInfoProvider).MustServe()
			log.WithField("ingress", cfg.Ingress.Kind).Infof("started proxying on %s", addr)
