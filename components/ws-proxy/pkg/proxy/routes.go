...
func NewRouteHandlerConfig(config *Config, opts ...RouteHandlerConfigOpt) (*RouteHandlerConfig, error) {
	corsHandler, err := corsHandler(config.NxpodInstallation.Scheme, config.NxpodInstallation.HostName)
	if err != nil {
		return nil, err
	}
...
func installTheiaRoutes(r *mux.Router, config *RouteHandlerConfig, rh *RouteHandlers) {
	...
	rh.theiaReadyHandler(r.Path("/nxpod/ready").Subrouter(), config)
...
// TheiaReadyHandler handles /nxpod/ready
