...
public class NxpodServerLauncher {
    ...
    private final NxpodClient client;

    private NxpodServerLauncher(
    ...
    public NxpodServerConnection listen(
            String apiUrl,
            String origin,
            String userAgent,
        ...
        String nxpodHost = URI.create(apiUrl).getHost();
        ...
        for (Proxy proxy : proxies) {
            ...
            if (!(proxyAddress instanceof InetSocketAddress)) {
                NxpodServerConnectionImpl.LOG.log(Level.WARNING, nxpodHost + ": unexpected proxy:", proxy);
                continue;
            }
        ...
        NxpodServerConnectionImpl connection = new NxpodServerConnectionImpl(nxpodHost);
        connection.whenComplete((input, exception) -> {
            try {
                container.stop();
            } catch (Throwable t) {
                NxpodServerConnectionImpl.LOG.log(Level.WARNING, nxpodHost + ": failed to stop websocket container:", t);
            }
        });
        ...
        connection.setSession(container.connectToServer(new Endpoint() {
            ...
            public void onError(Session session, Throwable thr) {
                NxpodServerConnectionImpl.LOG.log(Level.WARNING, nxpodHost + ": connection error:", thr);
    ...
    public static NxpodServerLauncher create(NxpodClient client) {
        return new Builder().create(client);
    }

    private static class Builder extends Launcher.Builder<NxpodServer> {

        public NxpodServerLauncher create(NxpodClient client) {
            setLocalService(client);
            setRemoteInterface(NxpodServer.class);
            ...
            return new NxpodServerLauncher(
                    messageReader,
                    jsonHandler,
                    remoteEndpoint,
                    messageWriter,
