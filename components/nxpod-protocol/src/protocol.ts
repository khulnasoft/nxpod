...
export interface ProfileDetails {
    ...
    jobRoleOther?: string;
    // Reasons user is exploring Nxpod when they signed up
...
export interface UserEnvVarValue extends EnvVarWithValue {
    id?: string;
    repositoryPattern: string; // DEPRECATED: Use ProjectEnvVar instead of repositoryPattern - https://github.com/nxpod-com/nxpod/issues/5322
}
...
export namespace UserEnvVar {
    ...
    export function validate(variable: UserEnvVarValue): string | undefined {
        ...
        if (!WhiteListFromReserved.includes(name) && name.startsWith("NXPOD_")) {
            return "Name with prefix 'NXPOD_' is reserved.";
        }
        if (name.trim() === "") {
...
export namespace WorkspaceImageBuild {
    export type Phase = "BaseImage" | "NxpodLayer" | "Error" | "Done";
...
/**
 * WithDefaultConfig contexts disable the download of the nxpod.yml from the repository
