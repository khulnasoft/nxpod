...
export interface NxpodServer extends JsonRpcServer<NxpodClient>, AdminServer, IDEServer {
    // User related API
    ...
    getNxpodTokenScopes(tokenHash: string): Promise<string[]>;
    ...
    updateOwnAuthProvider(params: NxpodServer.UpdateOwnAuthProviderParams): Promise<AuthProviderEntry>;
    deleteOwnAuthProvider(params: NxpodServer.DeleteOwnAuthProviderParams): Promise<void>;
    // org-level
    createOrgAuthProvider(params: NxpodServer.CreateOrgAuthProviderParams): Promise<AuthProviderEntry>;
    updateOrgAuthProvider(params: NxpodServer.UpdateOrgAuthProviderParams): Promise<AuthProviderEntry>;
    getOrgAuthProviders(params: NxpodServer.GetOrgAuthProviderParams): Promise<AuthProviderEntry[]>;
    deleteOrgAuthProvider(params: NxpodServer.DeleteOrgAuthProviderParams): Promise<void>;
    ...
    startWorkspace(id: string, options: NxpodServer.StartWorkspaceOptions): Promise<StartWorkspaceResult>;
    stopWorkspace(id: string): Promise<void>;
    deleteWorkspace(id: string): Promise<void>;
    setWorkspaceDescription(id: string, desc: string): Promise<void>;
    controlAdmission(id: string, level: NxpodServer.AdmissionLevel): Promise<void>;
    resolveContext(contextUrl: string): Promise<WorkspaceContext>;

    updateWorkspaceUserPin(id: string, action: NxpodServer.PinAction): Promise<void>;
    ...
    getOnboardingState(): Promise<NxpodServer.OnboardingState>;
    ...
    generateNewNxpodToken(options: NxpodServer.GenerateNewNxpodTokenOptions): Promise<string>;
...
export type DefaultImageSource =
    | "installation" // Source installation means the image comes from Nxpod instance install config
...
export const createServiceMock = function <C extends NxpodClient, S extends NxpodServer>(
    methods: Partial<JsonRpcProxy<S>>,
): NxpodServiceImpl<C, S> {
    return new NxpodServiceImpl<C, S>(createServerMock(methods));
};

export const createServerMock = function <S extends NxpodServer>(methods: Partial<JsonRpcProxy<S>>): JsonRpcProxy<S> {
...
export namespace NxpodServer {
    ...
    export interface GenerateNewNxpodTokenOptions {
        name?: string;
        type: NxpodTokenType;
        scopes?: string[];
    }
    export interface OnboardingState {
        /**
         * Whether this Nxpod instance is already configured with SSO.
         */
...
export class WorkspaceInstanceUpdateListener {
    ...
    constructor(private readonly service: NxpodService, private _info: WorkspaceInfo) {
...
export interface NxpodServiceOptions {
    onReconnect?: () => void | Promise<void>;
}

export class NxpodServiceImpl<Client extends NxpodClient, Server extends NxpodServer> {
    private readonly compositeClient = new NxpodCompositeClient<Client>();

    constructor(public readonly server: JsonRpcProxy<Server>, private options?: NxpodServiceOptions) {
