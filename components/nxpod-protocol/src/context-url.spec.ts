...
export class ContextUrlTest {
    @test public parseContextUrl_withEnvVar() {
        const actual = ContextURL.getNormalizedURL({
            contextURL: "passedin=test%20value/https://github.com/khulnasoft/nxpod-test-repo",
            context: {},
        } as WsContextUrl);
        expect(actual?.host).to.equal("github.com");
        expect(actual?.pathname).to.equal("/khulnasoft/nxpod-test-repo");
    }

    @test public parseContextUrl_withEnvVar_withoutSchema() {
        const actual = ContextURL.getNormalizedURL({
            contextURL: "passedin=test%20value/github.com/khulnasoft/nxpod-test-repo",
            context: {},
        } as WsContextUrl);
        expect(actual?.host).to.equal("github.com");
        expect(actual?.pathname).to.equal("/khulnasoft/nxpod-test-repo");
    ...
    @test public parseContextUrl_withEnvVar_sshUrl() {
        const actual = ContextURL.getNormalizedURL({
            contextURL: "passedin=test%20value/git@github.com:khulnasoft/nxpod-test-repo.git",
            context: {},
        } as WsContextUrl);
        expect(actual?.host).to.equal("github.com");
        expect(actual?.pathname).to.equal("/khulnasoft/nxpod-test-repo.git");
    }
