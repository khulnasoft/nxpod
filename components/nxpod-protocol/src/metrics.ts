/*---------------------------------------------------------------------------------------------
 *  Copyright (c) Nxpod. All rights reserved.
...
class PrometheusClientCallMetrics {
    ...
    constructor() {
        ...
        this.handledSecondsHistogram = new Histogram({
            ...
            labelNames: ["grpc_service", "grpc_method", "grpc_type", "grpc_code"],
            buckets: [0.1, 0.2, 0.5, 1, 2, 5, 10], // it should be aligned with https://github.com/khulnasoft/nxpod/blob/84ed1a0672d91446ba33cb7b504cfada769271a8/install/installer/pkg/components/ide-metrics/configmap.go#L315
            registers: [register],
        });
...
export class MetricsReporter {
    ...
    constructor(
        ...
        this.metricsHost = `ide.${new URL(options.nxpodUrl).hostname}`;
        if (typeof window !== "undefined") {
