...
func (b *DockerBuilder) getWorkspaceImageRef(ctx context.Context, baseref string, nxpodLayerHash string, allowedAuth allowedAuthFor) (ref string, err error) {
	span, ctx := opentracing.StartSpanFromContext(ctx, "getWorkspaceImageRef")
	defer tracing.FinishSpan(span, &err)

	cnt := []byte(fmt.Sprintf("%s\n%s\n%s", baseref, nxpodLayerHash, b.Config.ImageBuildSalt))
	hash := sha256.New()
