...
import (
	...
	"github.com/khulnasoft/nxpod/image-builder/api"
	"github.com/khulnasoft/nxpod/image-builder/pkg/resolve"
...
func (b *DockerBuilder) Start(ctx context.Context) (err error) {
	...
	hash, err := computeNxpodLayerHash(b.Config.NxpodLayerLoc)
	if err != nil {
		log.WithError(err).Error("Nxpod Layer hash computation failed")
		return err
	}
	b.gplayerHash = hash
	...
	log.WithField("ref", ref).WithField("nxpodLayerHash", b.gplayerHash).Info("self-build succeeded")
	b.builderref = ref
...
func (b *DockerBuilder) Build(req *api.BuildRequest, resp api.ImageBuilder_BuildServer) (err error) {
	...
	// TLDR; the getBaseImageRef call with request auth prevents users from pulling from our base image cache registry (e.g. eu.gcr.io/nxpod-dev)
	//
...
func (b *DockerBuilder) createBuildVolume(ctx context.Context, buildID string) (vol string, err error) {
	...
	initcontainer, err := b.Docker.ContainerCreate(ctx, &container.Config{
		...
		AttachStdin:  false,
		AttachStderr: true,
		Cmd:          []string{"sh", "-c", "cp -Rfv /nxpod-layer/scripts /workspace/scripts"},
		Tty:          false,
		Labels: map[string]string{
			LabelTemporary: "true",
		},
...
func (b *DockerBuilder) buildWorkspaceImage(ctx context.Context, bld *build, baseref string, targetref []string, allowedAuth allowedAuthFor) (err error) {
	...
	// TODO: rather than running those scripts as root we might want to create the directories they need and chown them to nxpod when creating the build volume
	...
	dfgenContainer, err := b.Docker.ContainerCreate(ctx, &container.Config{
		...
		AttachStderr: true,
		User:         "root",
		Cmd:          []string{"/nxpod-layer/scripts/generate-dockerfile.sh", baseref},
		WorkingDir:   ctxpth,
		Tty:          false,
		Labels: map[string]string{
			LabelTemporary: "true",
...
func (s *build) EnterPhase(p buildPhase) {
	...
	if p == buildPhaseBase {
		...
		fmt.Fprintf(s, "adding Nxpod layer\n")
	}
