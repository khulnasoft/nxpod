...
func TestDockerRegistryResolver(t *testing.T) {
	tests := []struct {
		...
		{"nxpod/workspace-full:build-branch-master", "docker.io/nxpod/workspace-full:build-branch-master", nil},
		{"nxpod/does-not-exist", "docker.io/nxpod/does-not-exist", fmt.Errorf("does not exist")},
	}
