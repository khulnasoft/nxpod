...
expected_uid="33333"
## Nxpod group
expected_group="nxpod"
expected_gid="33333"

# Disable repository protection
# HOTFIX(Krey): Results in `packages excluded due to repository priority protections` on nxpod for unknown reason
...
# Create Nxpod user
# INFO(Kreyren): This is stripped from https://github.com/jankeromnes/nxpod-layers/pull/3
...
if grep -q "$expected_user:.*:$expected_uid:$expected_gid:.*" /etc/passwd; then
	...
	if command -v useradd >/dev/null; then
		...
		useradd \
			...
			--comment "Created by $myName for nxpod usage" \
			"$expected_user" \
		...
		die 23 "Neither of expected commands useradd nor adduser are available on this environment, this usage of nxpod is unexpected and most likely malicious injection of altered dockerfile. Instance $NXPOD_WORKSPACE_ID with user $NXPOD_GIT_USER_EMAIL has been reported to upstream"
	else
