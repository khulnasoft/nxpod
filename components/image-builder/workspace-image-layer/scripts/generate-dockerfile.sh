...
DISTRO_PATH="$BASE_DIR/nxpod-layer/$DISTRO"
cp -R $DISTRO_PATH/. ./

# 3. Copy the nxpod-cli into the workspaace
cp $BASE_DIR/nxpod-cli nxpod-cli

# 4. Create Dockerfile in /workspace starting from BASE_IMAGE and append the distro-specific part
echo "FROM $BASE_IMAGE" > Dockerfile
(cat "$BASE_DIR/nxpod-layer/prepend.dockerfile"; echo) >> Dockerfile
