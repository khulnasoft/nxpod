...
import (
	...
	"github.com/khulnasoft/nxpod/content-service/pkg/git"
...
var (
	...
	OwnedByNxpod TestWorkspace = []TestWorkspaceEntry{
		// regular files with decent permissions
		{"foo", File, NxpodUID, NxpodGID, 0644, ""},
		{"bar", File, NxpodUID, NxpodGID, 0644, ""},

		// nested folders and files
		{"/one/level", Dir, NxpodUID, NxpodGID, 0744, ""},
		{"/one/level/file", File, NxpodUID, NxpodGID, 0744, ""},
		{"/two/levels/down", File, NxpodUID, NxpodGID, 0744, ""},

		// folders owned by nxpod with improper permissions, and their childen
		{"/subdir", Dir, NxpodUID, NxpodGID, 0700, ""},
		{"/subdir/content", File, NxpodUID, NxpodGID, 0600, ""},

		// circular symlinks
		{"/linktgt-nxpod", Dir, NxpodUID, NxpodUID, 00755, ""},
		{"/linktgt-nxpod/self-nxpod", Symlink, NxpodUID, NxpodGID, 0, "/linktgt-nxpod"},
	}

	OwnedByMixed TestWorkspace = []TestWorkspaceEntry{
		// regular files with decent permissions
		{"foo-root", File, 0, 0, 0644, ""},
		{"foo-nxpod", File, NxpodUID, NxpodGID, 0644, ""},

		// nested folders and files
		{"/one/root", File, 0, 0, 0644, ""},
		{"/one/nxpod", File, NxpodUID, NxpodGID, 0644, ""},
		{"/two/levels/root", File, 0, 0, 0644, ""},
		{"/two/levels/nxpod", File, NxpodUID, NxpodGID, 0644, ""},

		// hidden files and folders
		{"/.hidden-root/.content", File, 0, 0, 0755, ""},
		{"/.hidden-nxpod", Dir, NxpodUID, NxpodGID, 0744, ""},
		{"/.hidden-nxpod/.nxpod", File, NxpodUID, NxpodGID, 0744, ""},
		{"/.hidden-nxpod/.root", File, 0, 0, 0744, ""},

		// folders owned by root with improper permissions, and their childen
		{"/subdir-root", Dir, 0, 0, 0600, ""},
		{"/subdir-nxpod", Dir, NxpodUID, NxpodGID, 0600, ""},
		{"/subdir-root/root", File, 0, 0, 0600, ""},
		{"/subdir-root/nxpod", File, NxpodUID, NxpodGID, 0600, ""},
		{"/subdir-nxpod/root", File, 0, 0, 0600, ""},
		{"/subdir-nxpod/nxpod", File, NxpodUID, NxpodGID, 0600, ""},

		// circular symlinks
		{"/linktgt-root", Dir, 0, 0, 0755, ""},
		{"/linktgt-root/self-root", Symlink, 0, 0, 0, "/linktgt-root"},
		{"/linktgt-root/self-nxpod", Symlink, NxpodUID, NxpodGID, 0, "/linktgt-root"},
		{"/linktgt-nxpod", Dir, NxpodUID, NxpodGID, 0755, ""},
		{"/linktgt-nxpod/self-root", Symlink, 0, 0, 0, "/linktgt-nxpod"},
		{"/linktgt-nxpod/self-nxpod", Symlink, NxpodUID, NxpodGID, 0, "/linktgt-nxpod"},
	}
)
...
func TestInitFromBackup(t *testing.T) {
	...
	MessedUpPermissions := TestWorkspace{
		TestWorkspaceEntry{"/folder", Dir, NxpodUID, NxpodGID, 0400, ""},
		TestWorkspaceEntry{"/folder/file", File, NxpodUID, NxpodGID, 0400, ""},
		TestWorkspaceEntry{"/folder/really-broken-file", File, NxpodUID, NxpodGID, 0000, ""},
		TestWorkspaceEntry{"/maybe-broken/file", File, NxpodUID, NxpodGID, 0444, ""},
		TestWorkspaceEntry{"/broken-folder", Dir, NxpodUID, NxpodGID, 0000, ""},
		TestWorkspaceEntry{"/broken-folder/child", File, NxpodUID, NxpodGID, 0000, ""},
	}
	...
	testcases := map[string]initFromBackupTestcase{
		...
		"invalid-gh2428": {Workspace: TestWorkspace{
			TestWorkspaceEntry{"/folder", Dir, NxpodUID, NxpodGID, 0500, ""},
			TestWorkspaceEntry{"/folder/fuzz", Dir, NxpodUID, NxpodGID, 0500, ""},
			TestWorkspaceEntry{"/folder/fuzz/file", File, NxpodUID, NxpodGID, 0500, ""},
			TestWorkspaceEntry{"/folder/file", File, NxpodUID, NxpodGID, 0500, ""},
		}},
	}
...
func TestInitFromGit(t *testing.T) {
	...
	testcases := map[string]testInitFromGitTestcase{
		"valid-remote-head":   {OwnedByNxpod, "master", RemoteHead, "foobar"},
		"valid-remote-branch": {OwnedByNxpod, "after-commit", RemoteBranch, "foobar"},
		"valid-local-branch":  {OwnedByNxpod, "local-branch", LocalBranch, "foobar"},
	}
...
func TestInitFromPrebuild(t *testing.T) {
	...
	testcases := map[string]testInitFromPrebuildTestcase{
		...
		"with-nxpod":               {OwnedByNxpod, false, "", RemoteHead},
...
func (layout TestWorkspace) ValidateInitResult(t *testing.T, name, target string, expectWorkspace bool, workspacePrefix string) {
	err := filepath.Walk(target, func(path string, info os.FileInfo, err error) error {
		...
		if stat.Uid != NxpodUID || stat.Gid != NxpodGID {
			t.Errorf("post-init: %s is not owned by nxpod user but %d:%d", path, stat.Uid, stat.Gid)
		}
