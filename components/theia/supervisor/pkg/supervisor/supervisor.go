...
import (
	...
	"github.com/khulnasoft/nxpod/theia/supervisor/pkg/backup"
	"github.com/khulnasoft/nxpod/theia/supervisor/pkg/dropwriter"
...
Configuration is done using env vars:
	 ...
	 THEIA_WORKSPACE_ROOT  The location in the filesystem where the workspace lives.
	 	 NXPOD_REPO_ROOT  The absolute checkout location of the Git repo. If there's no Git repo, leave this empty.
        NXPOD_THEIA_PORT  The port Theia will listen on.
THEIA_SUPERVISOR_ENDPOINT  The address on which the supervisor health endpoint is served on.
	 ...
	 NXPOD_GIT_USER_NAME  Optional. Makes supervisor configure the global user.name Git setting.
	NXPOD_GIT_USER_EMAIL  Optional. Makes supervisor configure the global user.email Git setting.
...
type Config struct {
	...
	NxpodTheiaPort        int    `json:"theiaPort"`
...
func getConfig() (*Config, error) {
	...
	if cfg.RepoRoot == "" {
		cfg.RepoRoot = os.Getenv("NXPOD_REPO_ROOT")
	}

	if cfg.NxpodTheiaPort == 0 {
		gpTheiaPort := os.Getenv("NXPOD_THEIA_PORT")
		port, err := strconv.Atoi(gpTheiaPort)
		if err != nil {
			return nil, fmt.Errorf("cannot parse $NXPOD_THEIA_PORT (=%s): %v", gpTheiaPort, err)
		}
		cfg.NxpodTheiaPort = port
	}
	...
	cfg.Git.Name = os.Getenv("NXPOD_GIT_USER_NAME")
	cfg.Git.Email = os.Getenv("NXPOD_GIT_USER_EMAIL")
...
func prepareTheiaLaunch(cfg *Config) *exec.Cmd {
	...
	args = append(args, "--port", strconv.Itoa(cfg.NxpodTheiaPort))
...
func startContentInit(ctx context.Context, cfg *Config, wg *sync.WaitGroup, cst backup.ContentState) {
	...
	if os.IsNotExist(err) {
		...
		for range t.C {
			b, err := ioutil.ReadFile("/workspace/.nxpod/ready")
			if err != nil {
				if !os.IsNotExist(err) {
