...
import { TaskConfig } from '@nxpod/nxpod-protocol/lib/protocol';
import { WorkspaceReadyMessage, WorkspaceInitSource } from '@nxpod/nxpod-protocol/lib/wsready';
import { TerminalProcessInfo } from "../common/nxpod-info";
import { BackendApplicationContribution } from "@theia/core/lib/node";
import { NxpodFileParser } from '@nxpod/nxpod-protocol/lib/nxpod-file-parser';
import { FileUri } from '@theia/core/lib/node/file-uri';
import URI from "@theia/core/lib/common/uri";
import { TheiaHeadlessLogType } from '@nxpod/nxpod-protocol/lib/headless-workspace-log';
...
export class NxpodTaskStarter {
    ...
    @inject(NxpodFileParser) protected readonly nxpodFileParser: NxpodFileParser;
    ...
    async start(startPhase: StartPhase) {
        ...
        try {
            ...
            const isHeadless = process.env.NXPOD_HEADLESS === 'true';
            if (isHeadless) {
    ...
    protected addPrebuiltTasks(task: TaskConfig, index: number): TaskConfigWithPrebuild {
        ...
        const fileName = `/workspace/.nxpod/prebuild-log-${index}`;
        return {
            ...task,
    ...
    protected async getWorkspaceTasks(rootPath: string): Promise<TaskConfig[]> {
        try {
            try {
                const nxpodFile = path.join(rootPath, '.nxpod.yml');
                if (fs.existsSync(nxpodFile)) {
                    const contents = await fs.readFile(nxpodFile);
                    const parseResult = this.nxpodFileParser.parse(contents.toString());
                ...
                this.logger.info("Failed to parse tasks from local .nxpod.yml.", { err });
            }
            const values = JSON.parse(process.env.NXPOD_TASKS || '');
    ...
    protected async buildCmdStr(task: TaskConfigWithPrebuild, startPhase: StartPhase, index: number): Promise<string | undefined> {
        ...
        if (startPhase === "prebuild") {
            ...
            const fn = `/workspace/.nxpod/cmd-${index}`;
    ...
    protected watchHeadlessTask(term: TerminalProcess, rootPath: string): Promise<number> {
        ...
        // been output before.
        const logFile = `/workspace/.nxpod/prebuild-log-${term.id}`;
...
export class TheiaLocalTaskStarter implements BackendApplicationContribution {
    ...
    @inject(NxpodTaskStarter) starter: NxpodTaskStarter;
    onStart() {
        this.starter.start((process.env.NXPOD_START_MODE || "init") as StartPhase);
    }
}
...
export class WorkspaceReadyTaskStarter implements BackendApplicationContribution {
    ...
    async onStart() {
        const readyFilename = "/workspace/.nxpod/ready";
