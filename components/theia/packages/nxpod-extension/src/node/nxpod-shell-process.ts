...
@injectable()
export class NxpodShellProcess extends TerminalProcess {
    ...
    constructor(
        ...
        super(<TerminalProcessOptions>{
            ...
            options: {
                name: 'xterm-color',
                cols: options.cols || NxpodShellProcess.defaultCols,
                rows: options.rows || NxpodShellProcess.defaultRows,
                cwd: FileUri.fsPath(new URI(options.rootURI || process.env.THEIA_WORKSPACE_ROOT)),
                env: NxpodShellProcess.getEnvs(options.env || {}),
            }
...
export namespace NxpodShellProcess {
