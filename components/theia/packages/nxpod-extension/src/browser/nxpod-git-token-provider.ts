...
import { AuthProviderInfo } from "@nxpod/nxpod-protocol/lib/protocol";
...
@injectable()
export class NxpodGitTokenProvider {

    @inject(NxpodGitTokenValidator) protected tokenValidator: NxpodGitTokenValidator;
    ...
    protected async init() {
        ...
        const [nxpodInfo, authProviders] = await Promise.all([this.infoProvider.getInfo(), server.getAuthProviders()]);
        this.nxpodInfo = nxpodInfo;
     ...
     * @param tokenResult Nxpod user has already granted permissions, which needs to be checked here
     */
        ...
        if (gitCommand === "push") {
            ...
            if (!hasWriteAccess) {
                ...
                if (missingScopes.length > 0) {
                    ...
                    Please make sure Nxpod is authorized for the organization this repository belongs to.`;
    ...
    protected async showMessage(message: string, host: string, scopes?: string) {
        ...
        if (result === action) {
            const hostUrl = new NxpodHostUrl(this.nxpodInfo.host);
