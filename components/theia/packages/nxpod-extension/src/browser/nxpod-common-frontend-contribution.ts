...
@injectable()
export class NxpodCommonFrontendContribution extends CommonFrontendContribution {
    ...
    protected setThemeCookie(): void {
        ...
        const domain = new NxpodHostUrl(window.location.href).withoutWorkspacePrefix().url.host;
    ...
    protected async storeUserTheme(): Promise<void> {
        ...
        try {
            ...
            if (data) {
                ...
                await getNxpodService().server.updateUserStorageResource({
    ...
    protected async restoreUserTheme(): Promise<string | undefined> {
        ...
        try {
            ...
            const content = await getNxpodService().server.getUserStorageResource({ uri: currentThemeStorageUri });
            if (ThemeService.get().getTheme(currentId).id === currentId) {
