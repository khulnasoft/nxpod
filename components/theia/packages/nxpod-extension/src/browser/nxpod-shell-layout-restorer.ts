...
import { NxpodInfoService, TerminalProcessInfo } from "../common/nxpod-info";
...
@injectable()
export class NxpodLayoutRestorer extends ShellLayoutRestorer {
    ...
    protected doInitializeTerminals(shell: ApplicationShell, infos: TerminalProcessInfo[]) {
        ...
        const terminals: NxpodTerminalWidget[] = shell.widgets.filter(w => w instanceof NxpodTerminalWidget).map(t => t as NxpodTerminalWidget);
        ...
        for (const terminal of terminals) {
            const processId = (terminal as NxpodTerminalWidget).getTerminalId();
