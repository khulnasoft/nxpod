...
export class SetupManagerImpl implements SetupManager, FS, CommandContribution {
    ...
    static SETUP_PROJECT: Command = {
        id: 'setup-project',
        label: 'Nxpod: Setup Project',
    };

    static ADD_BADGE: Command = {
        id: 'add-badge',
        label: 'Nxpod: Add Ready-To-Code Badge to README',
    };

    static CHANGE_DOCKER_IMAGE: Command = {
        id: 'change-image',
        label: 'Nxpod: Change Image Configuration in .nxpod.yml'
    }

    static TEST_RUN_SETUP: Command = {
        id: 'test-run-setup',
        label: 'Nxpod: Test the Setup'
    }

    @inject(FileService) private readonly fs: FileService;
    @inject(NxpodInfoService) private readonly infoService: NxpodInfoService;
    ...
    async testSetup(): Promise<void> {
        await this.withProgress('Test Setup',
            async (progress, cancellationToken) => {
                if (!this.exists(this.nxpodYml)) {
                    this.messageService.error('Cannot test run the configuration. No .nxpod.yml found.');
                    return;
                }
                const myLogin = await this.gitHubModel.getMyLogin();
                const testbranch = myLogin + '/nxpod-setup';
                const total = 6;
                let done = 1;
                ...
                const gpUri = await this.toURI(this.nxpodYml);
...
`);
    ...
    async hasConfig(): Promise<boolean> {
        const existsConfig = await this.exists(this.nxpodYml);
        return existsConfig;
    }
    ...
    async createAndOpenConfig(): Promise<EditorWidget> {
        return await this.withProgress('Create .nxpod.yml',
            async (progress, cancellationToken) => {
                ...
                progress.report({
                    message: 'Updating .nxpod.yml ...',
                    work: {
                        done: 40,
                        total: 50
                    }
                });
                let shouldWrite: boolean | undefined = ! await this.exists(this.nxpodYml);
                if (!shouldWrite) {
                    shouldWrite = await this.quickPick.show<boolean>([
                        {
                            label: 'Overwrite existing .nxpod.yml',
                            value: true
                        },
                        {
                            label: 'Abort',
                        ...
                        title: 'A .nxpod.yml file already exists. Do you want to overwrite?'
                    });
                }
                if (shouldWrite) {
                    await this.write(this.nxpodYml, YAML.stringify(config));
                }
                progress.report({
                ...
                this.messageService.info('Sucessfully created an initial .nxpod.yml.');
                return this.open(this.nxpodYml, { mode: 'activate' });
            }
        );
    }
    ...
    async updateDockerConfig(): Promise<EditorWidget | undefined> {
        ...
        if (result === 'custom') {
            ...
            this.quickOpenService.open({
                onType(lookFor: string, acceptor: (items: QuickOpenItem[]) => void): void {
                    acceptor([
                        new QuickOpenItem({
                            label: `Use docker image with name: "${lookFor || 'nxpod/workspace-full'}". Press 'Enter' to confirm or 'Escape' to cancel.`,
                            run: () => {
                                result = lookFor || 'nxpod/workspace-full';
                                return true;
                            }
                        })
                    ]);
                ...
                placeholder: 'The Docker image name to use as a base (defaults to \'nxpod/workspace-full\')',
                fuzzySort: false,
                onClose: () => {
                    wait.resolve();
                }
        ...
        const customDocker = await this.quickPick.show<boolean>([{
            ...
            label: 'Yes, generate a .nxpod.Dockerfile',
            value: true
        }], {
            title: 'Do you want to install additional tools (i.e. generate a .nxpod.Dockerfile)?'
        })
    ...
    private async changeDockerImage(baseImage: string = 'nxpod/workspace-full', dockerFile: boolean = false): Promise<EditorWidget> {
        if (!await this.hasConfig()) {
            await this.createAndOpenConfig();
        }
        if (baseImage === 'nxpod/workspace-full' && !dockerFile) {
            this.messageService.info('You picked the default. No need to update .nxpod.yml.');
            await this.open(this.nxpodYml, { mode: 'activate' });
        }
        return await this.withProgress('Change Docker',
            async (progress, cancellationToken) => {
                ...
                if (!parsedDoc || !parsedDoc.contents) {
                    throw new Error("Couldn't parse " + this.nxpodYml);
                }
                if (dockerFile) {
                    const imageConfig = { "file": this.dockerFileName };
                    if ((parsedDoc as any).get('image')) {
                        (parsedDoc as any).set('image', imageConfig);
                        await this.write(this.nxpodYml, parsedDoc.toString());
                    } else {
                        await this.write(this.nxpodYml, YAML.stringify({image: imageConfig}) + '\n' + parsedDoc.toString());
                    }
                    const nxpodEditor = await this.open(this.nxpodYml);
...
`);
                    ...
                    this.messageService.info('Sucessfully created .nxpod.Dockerfile. Please customize it to your needs.');
                    return await this.open(this.dockerFileName, { mode: 'activate', widgetOptions: { mode: 'split-right', ref: nxpodEditor } });
                } else {
                    if ((parsedDoc as any).get('image')) {
                        (parsedDoc as any).set('image', baseImage);
                        await this.write(this.nxpodYml, parsedDoc.toString());
                    } else {
                        await this.write(this.nxpodYml, YAML.stringify({image: baseImage}) + '\n' + parsedDoc.toString());
                    }
                    this.messageService.info('Sucessfully updated .nxpod.yml.');
                    return await this.open(this.nxpodYml, { mode: 'activate' });
                }
            });
    }
    ...
    async updateReadme(): Promise<EditorWidget> {
        ...
        this.write(fileName,
            `[![Nxpod ready-to-code](https://img.shields.io/badge/Nxpod-ready--to--code-blue?logo=nxpod)](${contextUrl})
