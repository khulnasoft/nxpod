...
import { PortConfig, PortOnOpen, PortRangeConfig, WorkspaceInstance, NxpodService, Queue, PortVisibility, WorkspaceInstancePort } from "@nxpod/nxpod-protocol";
import { Emitter, Event } from "@theia/core";
import { Deferred } from "@theia/core/lib/common/promise-util";
import { NxpodFileParser } from "@nxpod/nxpod-protocol/lib/nxpod-file-parser";

import debounce = require('lodash.debounce');
import { NxpodPortsAuthManger } from "./nxpod-ports-auth-manager";
...
@injectable()
export class NxpodPortsService {
    ...
    @inject(NxpodFileParser) protected nxpodConfigParser: NxpodFileParser;
    @inject(NxpodPortsAuthManger) protected readonly portsAuthManager: NxpodPortsAuthManger;
    ...
    protected async init() {
        ...
        this.configUri = 'file://' + info.repoRoot + '/.nxpod.yml';

        // listen on FS change events asap
        this.fileService.onDidFilesChange(async change => {
            if (change.changes.some(c => c.resource.toString().endsWith('/.nxpod.yml'))) {
    ...
    protected async updateLocalPortConfig() {
        ...
        try {
            ...
            const parseResult = this.nxpodConfigParser.parse(contents.value, { acceptPortRanges: true });
            ...
            for (const config of source) {
                if (PortRangeConfig.is(config)) {
                    ...
                    if (range) {
                        ...
                        console.error('Please fix this port range in your .nxpod.yml:', config);
                    }
                }
            }
        }
