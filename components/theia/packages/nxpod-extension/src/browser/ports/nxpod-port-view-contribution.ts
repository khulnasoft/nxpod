...
import { NxpodPortViewWidget, PORT_WIDGET_FACTORY_ID } from "./nxpod-port-view-widget";
import { NxpodPortsService, PortChangeEvent, PortChange } from "./nxpod-ports-service";
import { MiniBrowserOpenHandler } from "@theia/mini-browser/lib/browser/mini-browser-open-handler";
import { ServedPort } from "../../common/served-ports-service";
import { PortConfig } from "@nxpod/nxpod-protocol";
...
export class NxpodPortViewContribution extends AbstractViewContribution<NxpodPortViewWidget> implements FrontendApplicationContribution {
    ...
    constructor() {
        super({
            ...
            widgetName: NxpodPortViewWidget.LABEL,
    ...
    protected async onServedPortsChanged({ didOpen }: PortChange<ServedPort>): Promise<void> {
        for (const servedPort of didOpen) {
            ...
            // Config is either:
            //  - statically configured (in the .nxpod.yml)
            ...
            if (config && isPersisted) {
                // These are ports with a config in .nxpod.yml (either all along, or locally in the workspace)
                if (config.onOpen === 'ignore') {
    ...
    protected showOpenServiceNotification(servedPort: ServedPort, offerMakePublic: boolean = false) {
        ...
        //  - "Always Ignore"
        // see https://github.com/TypeFox/nxpod/issues/3892
    ...
    protected async updateStatusBar() {
        ...
        this.statusBar.setElement(NxpodPortViewContribution.PORTS_OPEN_PORTS, {
            text,
