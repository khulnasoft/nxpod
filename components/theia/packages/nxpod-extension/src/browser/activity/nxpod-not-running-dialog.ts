...
import { WorkspaceInstance, NxpodClient } from "@nxpod/nxpod-protocol";
...
import { formatHours } from "@nxpod/nxpod-protocol/lib/util/date-time";
import { NxpodInfo } from "../../common/nxpod-info";

export class NxpodNotRunningOverlay extends BaseWidget {
    ...
    constructor(service: NxpodService, info: NxpodInfo, messageService: MessageService, connectionStatus: ConnectionStatusService) {
        super();
        ...
        this.startAction = () => {
            const startWsUrl = new NxpodHostUrl(info.host).with({
                pathname: '/start/',
        ...
        makeLink(dashboardButton, new NxpodHostUrl(info.host).asDashboard().toString(), 'Workspaces');
        ...
        try {
            ...
            const onCreditAlert = (creditAlert: any /* CreditAlert */) => {
                if (creditAlert.remainingUsageHours <= 0) {
                    spinner.style.visibility = 'hidden';
                    startButton.style.visibility = 'hidden';
                    titleNode.textContent = 'Nxpod Credit Alert';
                    status.textContent = 'You have run out of Nxpod Hours.';
                    contextUrlButton.onclick = () => {
                        const url = new NxpodHostUrl(info.host).asUpgradeSubscription().toString();
    ...
    private async getContextUrl(info: NxpodInfo, service: NxpodService): Promise<string> {
