...
import { ResolvedPluginKind } from '@nxpod/nxpod-protocol';
import { ExtensionsInstaller } from './extensions-installer';
import { DeployedPlugin } from '../../common/nxpod-plugin-service';
import { NxpodPluginData } from './nxpod-plugin-support';
import { ExtensionNodes } from './extensions-source';

@injectable()
export class NxpodVSXExtension extends VSXExtension {
    ...
    get deployedPlugin(): DeployedPlugin | undefined {
        const data: NxpodPluginData = this.pluginSupport as any;
    ...
    render(): React.ReactNode {
        return <NxpodExtensionComponent extension={this} />;
    }

    renderEditor(): React.ReactNode {
        return <NxpodExtensionEditorComponent extension={this} />;
    }

}

class NxpodExtensionComponent extends VSXExtensionComponent {
    protected renderAction(): React.ReactNode {
        return renderAction(this);
    }
}

class NxpodExtensionEditorComponent extends VSXExtensionEditorComponent {
    protected renderAction(): React.ReactNode {
        return renderAction(this);
    }
}
...
const renderAction = (component: AbstractVSXExtensionComponent) => {
    const ext = component.props.extension as NxpodVSXExtension;
...
const updateVersion = async (extension: NxpodVSXExtension, component: AbstractVSXExtensionComponent) => {
    try {
