...
import { DeployedPlugin, NxpodPluginModel, NxpodPluginService } from '../../common/nxpod-plugin-service';
import { HostedPluginSupport } from '@theia/plugin-ext/lib/hosted/browser/hosted-plugin';
import { NxpodPluginSupport, NxpodPluginData } from './nxpod-plugin-support';
import { ResolvedPluginKind, UserInfo } from '@nxpod/nxpod-protocol';
...
export class ExtensionNodes {
    ...
    protected readonly pluginSupport: NxpodPluginSupport;
    ...
    *getNodes(pluginKind: ResolvedPluginKind): IterableIterator<ExtensionNode> {
    ...
    protected async init(): Promise<void> {
        const [nxpodInfo, { server }] = await Promise.all([
            this.infoProvider.getInfo(),
            this.serviceProvider.getService()
        ]);
        const [workspaceOwner, isWorkspaceOwner] = await Promise.all([
            server.getWorkspaceOwner(nxpodInfo.workspaceId),
            server.isWorkspaceOwner(nxpodInfo.workspaceId)
        ])
    ...
    protected update(): void {
        const data: NxpodPluginData = this.pluginSupport;
        this._loading = false;
...
export class ExtensionNode implements TreeElement {
    ...
    render(): React.ReactNode {
        const { plugin } = this;
        let { icon, author, publisher, name, displayName } = plugin.model as NxpodPluginModel;
        ...
        return <div className='nxpod-extension'>
            <div className='nxpod-extension-icon' style={iconStyle} />
            <div className='nxpod-extension-content'>
                <div><span className='nxpod-extension-ellipsis nxpod-extension-name'>{displayName}</span> <span className='nxpod-extension-version'>{plugin.model.version}</span></div>
                <div className='nxpod-extension-ellipsis nxpod-extension-description'>{plugin.model.description}</div>
                <div className='nxpod-extension-action-bar'>
                    <span className='nxpod-extension-ellipsis nxpod-extension-publisher'>{publisher}</span>
                    {this.renderAction()}
                </div>
            </div>
        </div >
    ...
    protected renderAction(): React.ReactNode {
        ...
        if (!this.installed) {
            return <button className="theia-button nxpod-extension-action" title='reload window to uninstall' onClick={this.props.onReload}>Reload</button>;
        }
        return <button className="theia-button nxpod-extension-action" onClick={this.props.onUninstall}>Uninstall</button>;
    }
