...
import { GetGitTokenResult } from "./nxpod-git-token-provider";
...
@injectable()
export class NxpodGitTokenValidator {
    ...
    async checkWriteAccessForGitHubRepo(token: string, host: string, repoFullName: string): Promise<CheckWriteAccessResult> {
        ...
        if (inOrg) {
            // if this repository belongs to an organization and Nxpod is not authorized,
