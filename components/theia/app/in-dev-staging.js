...
function runInDevStaging(command, verbose) {
    const repo = 'https://github.com/khulnasoft/spring-petclinic';
    ...
    if (proxyName === '') {
        ...
        console.error('Deploy : https://werft.nxpod-dev.com/jobs');
        return;
    }
    ...
    if (podName.trim() === '') {
        ...
        console.error(`http://${currentBranch.toLowerCase().replace(/[^a-z]/g,'-')}.staging.nxpod-dev.com/#${repo}`);
        return;
    }
...
`    # Patched for Development Mode
    ...
    result.stdout.on('data', data => {
        ...
        if (strData.indexOf('listening') !== -1) {
            ...
            run(`gp preview ${pod.metadata.annotations["nxpod/url"]}/#/workspace/${folder}`);
        }
    });
