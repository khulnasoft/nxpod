...
// NewFromEnvironment creates a new analytics writer based on the NXPOD_ANALYTICS_WRITER
...
func NewFromEnvironment() Writer {
	switch os.Getenv("NXPOD_ANALYTICS_WRITER") {
	case "log":
		log.Debug("log analytics")
		return &logAnalyticsWriter{}
	case "segment":
		log.Debug("segment analytics")
		client, err := segment.NewWithConfig(os.Getenv("NXPOD_ANALYTICS_SEGMENT_KEY"), segment.Config{
			Endpoint: os.Getenv("NXPOD_ANALYTICS_SEGMENT_ENDPOINT"),
		})
		if err != nil {
