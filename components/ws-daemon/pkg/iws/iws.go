...
import (
	...
	"github.com/khulnasoft/nxpod/common-go/cgroups"
	v2 "github.com/khulnasoft/nxpod/common-go/cgroups/v2"
	"github.com/khulnasoft/nxpod/common-go/log"
	"github.com/khulnasoft/nxpod/common-go/tracing"
	wsinit "github.com/khulnasoft/nxpod/content-service/pkg/initializer"
	"github.com/khulnasoft/nxpod/ws-daemon/api"
	"github.com/khulnasoft/nxpod/ws-daemon/pkg/container"
	"github.com/khulnasoft/nxpod/ws-daemon/pkg/internal/session"
	nsi "github.com/khulnasoft/nxpod/ws-daemon/pkg/nsinsider"
)
...
func (wbs *InWorkspaceServiceServer) PrepareForUserNS(ctx context.Context, req *api.PrepareForUserNSRequest) (*api.PrepareForUserNSResponse, error) {
	...
	err = nsi.Nsinsider(wbs.Session.InstanceID, int(containerPID), func(c *exec.Cmd) {
		c.Args = append(c.Args, "prepare-dev", "--uid", strconv.Itoa(wsinit.NxpodUID), "--gid", strconv.Itoa(wsinit.NxpodGID))
	})
	if err != nil {
...
// └── workspace       drwxr-xr-x 5 nxpodUid nxpodGid
//
//	└── user        drwxr-xr-x 5 nxpodUid nxpodGid
func (wbs *InWorkspaceServiceServer) EvacuateCGroup(ctx context.Context, req *api.EvacuateCGroupRequest) (*api.EvacuateCGroupResponse, error) {
	...
	out, err := exec.CommandContext(ctx, "chown", "-R", fmt.Sprintf("%d:%d", wsinit.NxpodUID, wsinit.NxpodGID), filepath.Join(wbs.CGroupMountPoint, workspaceCGroup)).CombinedOutput()
	if err != nil {
