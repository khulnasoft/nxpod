...
func TestDownloadManifest(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			Expectation: func(url string) Expectation {
				return Expectation{
					Error: "cannot download manifest from " + url + NxpodCLIBasePath + "/manifest.json: 404 Not Found",
				}
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			if test.Manifest != nil {
				mux.Handle(filepath.Join(NxpodCLIBasePath, "/manifest.json"), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestReplaceSelf(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			mux := http.NewServeMux()
			mux.Handle(NxpodCLIBasePath+"/manifest.json", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				_ = json.NewEncoder(w).Encode(mf)
			}))
			for _, f := range test.Files {
				mux.Handle(NxpodCLIBasePath+"/"+f.Filename, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
