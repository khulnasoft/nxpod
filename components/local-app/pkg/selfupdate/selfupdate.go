...
const (
	// NxpodCLIBasePath is the path relative to a Nxpod installation where the latest
	// binary and manifest can be found.
	NxpodCLIBasePath = "/static/bin"
)
...
func DownloadManifest(ctx context.Context, baseURL string) (res *Manifest, err error) {
	defer func() {
		if err != nil {
			pth := strings.TrimSuffix(baseURL, "/") + NxpodCLIBasePath
	...
	murl.Path = filepath.Join(murl.Path, NxpodCLIBasePath)
