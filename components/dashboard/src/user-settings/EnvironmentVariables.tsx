...
import { UserEnvironmentVariable } from "@nxpod/public-api/lib/nxpod/v1/envvar_pb";
...
function AddEnvVarModal(p: EnvVarModalProps) {
    ...
    return (
        <Modal visible={true} onClose={p.onClose} onSubmit={save}>
            ...
            <ModalBody>
                {error ? (
                    <div className="bg-kumquat-light rounded-md p-3 text-nxpod-red text-sm mb-2">{error}</div>
                ) : null}
                <TextInputField
                    label="Name"
                    value={ev.name}
