...
import { NxpodService, UserPlatform } from "@nxpod/nxpod-protocol";
...
export namespace browserExtension {
    ...
    export async function getUserPlatform(service: NxpodService): Promise<UserPlatform | undefined> {
    ...
    export async function updatePlatformInformation(service: NxpodService): Promise<void> {
        const user = await service.server.getLoggedInUser();
        ...
        if (data && data.platforms) {
            // limit length to max 5 entries, due to https://github.com/TypeFox/nxpod/issues/4214
    ...
    function isBrowserExtensionInstalled(): boolean | undefined {
        ...
        if (window.parent === window) {
            const ele = document.getElementById('ExtensionCheck_NxpodBrowserExtension');
            if (ele) {
