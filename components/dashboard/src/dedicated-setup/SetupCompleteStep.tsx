...
export const SetupCompleteStep: FC<Props> = ({ onComplete }) => {
        ...
        copyToClipboard(url)
            ...
            .catch((error) => {
                if (error instanceof DOMException) {
                    setCopyError(
                        "Nxpod is not allowed to copy to clipboard. Please copy the URL manually or adjust your browser permissions.",
                    );
                    return;
                }
    ...
    return (
        <SetupLayout showOrg noMaxWidth>
            <Heading1>Welcome to Nxpod</Heading1>
            <Subheading>Your teammates can now sign in to Nxpod using single sign-on (SSO).</Subheading>
