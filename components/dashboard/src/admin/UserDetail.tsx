...
export default function UserDetail(p: { user: User }) {
    ...
    const verifyUser = async () => {
        await updateUser(async (u) => {
            return await getNxpodService().server.adminVerifyUser(u.id);
        });
    };
    ...
    const toggleBlockUser = async () => {
        await updateUser(async (u) => {
            u.blocked = !u.blocked;
            await getNxpodService().server.adminBlockUser({
                blocked: u.blocked,
                id: u.id,
            });
            return u;
    ...
    const deleteUser = async () => {
        await updateUser(async (u) => {
            ...
            await getNxpodService().server.adminDeleteUser(u.id);
            return u;
        });
    };
    ...
    return Object.entries(WorkspaceFeatureFlags)
        ...
        .map((name) => {
            ...
            return {
                ...
                onClick: async () => {
                    await updateUser(async (u) => {
                        return await getNxpodService().server.adminModifyPermanentWorkspaceFeatureFlag({
                            id: user.id,
                            changes: [
                                {
                                    featureFlag: name,
...
function getRopEntries(user: User, updateUser: UpdateUserFunction): Entry[] {
    const createRopEntry = (name: RoleOrPermission, role?: boolean) => {
        ...
        return {
            ...
            onClick: async () => {
                await updateUser(async (u) => {
                    return await getNxpodService().server.adminModifyRoleOrPermission({
                        id: user.id,
                        rpp: [
                            {
                                r: name,
