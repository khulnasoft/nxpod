...
import { AdditionalUserData, User as UserProtocol } from "@nxpod/nxpod-protocol";
...
export const useUpdateCurrentUserMutation = () => {
    return useMutation({
        mutationFn: async (partialUser: UpdateCurrentUserArgs) => {
            const current = await getNxpodService().server.getLoggedInUser();
            ...
            const user = await getNxpodService().server.updateLoggedInUser(update);
