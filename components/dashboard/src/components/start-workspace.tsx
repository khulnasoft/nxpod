...
import { NxpodService, NxpodClient, WorkspaceInstance, Disposable, WorkspaceInstanceStatus, WorkspaceImageBuild, WithPrebuild, Branding, Workspace } from '@nxpod/nxpod-protocol';
...
import { HeadlessLogEvent } from '@nxpod/nxpod-protocol/lib/headless-workspace-log';
...
export type StartErrorRenderer = (errorCode: number, service: NxpodService, onResolved: () => void) => JSX.Element | undefined;

export class StartWorkspace extends React.Component<StartWorkspaceProps, StartWorkspaceState> implements NxpodClient {
        ...
        if (workspaceInstance.status.phase === 'stopping') {
            if (this.isHeadless) {
                if (this.workspace) {
                    ...
                    this.redirectTo(new NxpodHostUrl(window.location.toString()).withContext(ctxUrl).toString());
                } else {
    ...
    protected async ensureWorkspaceAuth(instanceID: string) {
        if (!document.cookie.includes(`${instanceID}_owner_`)) {
            const authURL = new NxpodHostUrl(window.location.toString()).asWorkspaceAuth(instanceID);
    ...
    protected redirectToDashboard() {
        const url = new NxpodHostUrl(window.location.toString()).asDashboard().toString();
        this.redirectTo(url);
    }
