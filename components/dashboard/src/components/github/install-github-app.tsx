...
export class InstallGithubApp extends React.Component<{}, InstallGithubAppState> {
    private service = createNxpodService();
            ...
            if (e instanceof ResponseError) {
                switch (e.code) {
                    ...
                    case ErrorCodes.NOT_AUTHENTICATED:
                        let url = new NxpodHostUrl(window.location.toString()).withApi({
                            pathname: '/login/',
    ...
    render() {
                        ...
                        <Grid item xs={12}>
                            ...
                            <List>
                                <ListItem style={{ paddingLeft: '0px' }}>
                                    <ListItemText>
                                        You are about to install the GitHub app for Nxpod.
                                        That's a great idea because it makes Nxpod even better for you and everyone else who contributes to your repositories.
                                    </ListItemText>

                                </ListItem>
                                <ListItem style={{ paddingLeft: '0px' }}>
                                    <ListItemText>
                                        <b>Note:</b> If you want to use prebuilt workspaces for private repositories, your Nxpod user needs access to those repositories.
                                    Use the <a href="/access-control/" target="_blank" rel="noopener">Access Control</a> page to configure what Nxpod has access to.
                                    </ListItemText>
                                </ListItem>
                            </List>
                        </Grid>
        ...
        const thisUrl = new NxpodHostUrl(new URL(window.location.toString()));
        return (
            <React.Fragment>
                {content}
                <Dialog open={this.state.showDialog === 'cancel'}>
                    ...
                    <DialogContent>
                        <DialogContentText>Canceling the installation here just prevents Nxpod from reacting to the events it receives from GitHub.
                        To fully undo the installation, please uninstall the <a href={`https://github.com/settings/installations/${this.installationId}`} target="_blank" rel="noopener noreferrer">Nxpod app in GitHub</a>.</DialogContentText>
                    </DialogContent>
                    <DialogActions>
                ...
                <Dialog open={this.state.showDialog === 'errorWhenReg'}>
                    ...
                    <DialogContent>
                        <DialogContentText>Could not register the app with Nxpod - there seems to be a network issue. Please try again in a moment.</DialogContentText>
                    </DialogContent>
                    <DialogActions>
