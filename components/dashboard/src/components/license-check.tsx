...
import { LicenseValidationResult, LicenseService } from '@nxpod/nxpod-protocol/lib/license-protocol';
...
export class LicenseCheck extends React.Component<LicenseCheckProps, LicenseCheckState> {
    ...
    public render() {
        ...
        let classname = 'valid';
        if (!result.valid) {
            const licenseLink = new NxpodHostUrl(window.location.toString()).with({ pathname: "license" }).toString();

            message = (<React.Fragment>
                <p><strong>Your Nxpod license is invalid.</strong></p>
