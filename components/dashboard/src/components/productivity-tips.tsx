...
import { UserPlatform } from '@nxpod/nxpod-protocol';
...
export class ProductivityTips extends React.Component<{userHasCreatedWorkspaces?: boolean}, { platform?: UserPlatform }> {
    ...
    constructor(props: {userHasCreatedWorkspaces?: boolean, shouldRenderTips: boolean}) {
        ...
        browserExtension.getUserPlatform(createNxpodService()).then(
            platform =>
            this.setState({
                platform
            })
        ...
        if (!isChrome && !isFirefox) {
            message = (
                <div className='pro-tip message'>
                    Nxpod runs best on <a href='https://www.google.com/chrome/' target='_blank' rel='noopener noreferrer'>Chrome</a> and <a href='https://www.mozilla.org/firefox/' target='_blank' rel='noopener noreferrer'>Firefox</a>.
				</div>
            );
                ...
                <div className='pro-tip message'>
                    <div className='background'>
                        ...
                        <div className='image'>
                            ...
                            <img width='600' height='27' src={`/images/tip-nxpod-url-blank2.png`} />
                        </div>
                    </div>
                </div>
            </React.Fragment>;
        } else if (this.hasNxpodExtension() === false) {
            const extensionButton = isChrome ?
                <Button className='button' variant='outlined' color='secondary'
                    href='https://chrome.google.com/webstore/detail/nxpod-online-ide/dodmmooeoklaejobgleioelladacbeki' target='_blank' rel='noopener noreferrer'>
                    Get Chrome Extension
				</Button> :
                <Button className='button' variant='outlined' color='secondary'
                    href='https://addons.mozilla.org/firefox/addon/nxpod/' target='_blank' rel='noopener noreferrer'>
    ...
    protected hasNxpodExtension(): boolean | undefined {
...
class UrlAnimation extends React.Component<{}, { url: string }>{
    ...
    protected writeALetter = ((timestamp: number) => {
        ...
        if ((this.prefix === '' && progress > this.startDelay) || (this.prefix !== '' && progress > this.stepTime)) {
            this.prefix = this.nxpodUrl.substring(0, this.prefix.length+1);
            this.setState({ url: this.prefix });
            this.start = timestamp;
            this.stepTime = this.initialStepTime + (100 * Math.random());
        }
        if (this.prefix !== this.nxpodUrl) {
