...
import { NxpodService, NxpodToken, NxpodTokenType, Permission } from '@nxpod/nxpod-protocol';
...
export class ApiTokenView extends React.Component<ApiTokenViewProps, ApiTokenViewState> {
    ...
    protected async generateNewToken(tokenName: string | undefined) {
        const token = await this.props.service.server.generateNewNxpodToken({ name: tokenName, type: NxpodTokenType.API_AUTH_TOKEN });
        this.setState({ token });
    ...
    protected async deleteToken(tokenHash: string) {
        try {
            await this.props.service.server.deleteNxpodToken(tokenHash);
            await this.updateTokens();
        } catch (e) {
    ...
    protected async updateTokens() {
        const tokens = ((await this.props.service.server.getNxpodTokens())||[]).filter(token => token.type === NxpodTokenType.API_AUTH_TOKEN);
