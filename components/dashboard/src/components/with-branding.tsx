...
import { NxpodService, Branding } from '@nxpod/nxpod-protocol';
...
export function updateBrowserTab(branding: Branding | undefined = WithBranding.getCachedBranding()) {
    ...
    window.document.title = window.document.title.replace("Nxpod", branding.name);
...
export class WithBranding extends React.Component<WithBrandingProps, WithBrandingState> {
    ...
    static async getBranding(service?: NxpodService, forceRefresh?: boolean): Promise<Branding | undefined> {
