...
import { Workspace, WorkspaceInstance, NxpodService, WorkspaceInstancePhase, Configuration } from '@nxpod/nxpod-protocol';
...
import { getNxpodConfiguration } from '../configuration';
...
export default class WorkspaceEntry extends React.Component<WorkspaceEntryProps, WorkspaceEntryState> {
    ...
    constructor(props: WorkspaceEntryProps) {
        ...
        getNxpodConfiguration().then(configuration => this.setState({ configuration }));
    }
    ...
    render(): React.ReactNode {
        ...
        if (status === 'stopped') {
            const downloadURL = new NxpodHostUrl(window.location.href).with({ pathname: `/workspace-download/get/${this.props.workspace.id}` }).toString();
            buttons.push(
                <Button key='stop' className='button' variant='outlined' color='primary' href={downloadURL} title={downloadTooltip}>
                    Download
                </Button>
            )
            const startUrl = new NxpodHostUrl(window.location.href).with({
                pathname: '/start/',
        ...
        if (repo) {
            ...
            const nxpodProtocolDoesNotProvideTotalChanges = repo.totalUncommitedFiles == undefined && repo.totalUntrackedFiles == undefined && repo.totalUnpushedCommits == undefined;
            if (nxpodProtocolDoesNotProvideTotalChanges) {
                // we have an old workspace that does not provide the total changes via Nxpod protocol
