...
import { User, AuthProviderInfo, Branding } from '@nxpod/nxpod-protocol';
...
export namespace MenuEntry {
    export const ACCESS_CONTROL: MenuEntry = {
        ...
        open: (loc) => new NxpodHostUrl(loc.toString()).asAccessControl().toString()
    };
    export const SETTINGS: MenuEntry = {
        caption: 'Settings',
        open: (loc) => new NxpodHostUrl(loc.toString()).asSettings().toString()
    };
    export const LOG_OUT: MenuEntry = {
        caption: 'Log Out',
        open: (loc) => new NxpodHostUrl(loc.toString()).withApi({ pathname: '/logout' }).toString()
    };
...
export class MainMenu extends React.Component<MainMenuProps, MainMenuState> {
    ...
    protected doLogin(host: string) {
        window.location.href = new NxpodHostUrl(window.location.href).withApi({
            pathname: '/login/',
            search: `host=${host}`
        }).toString();
    }
