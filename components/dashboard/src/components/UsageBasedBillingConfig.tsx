...
export default function UsageBasedBillingConfig({ hideSubheading = false }: Props) {
    ...
    useEffect(() => {
        if (attributionId) {
            getNxpodService().server.getPriceInformation(attributionId).then(setPriceInformation);
        }
    }, [attributionId]);
    ...
    const refreshSubscriptionDetails = useCallback(
        async (attributionId: string) => {
            ...
            try {
                getNxpodService().server.getStripePortalUrl(attributionId).then(setStripePortalUrl);
                getNxpodService().server.getUsageBalance(attributionId).then(setCurrentUsage);
                const costCenter = await getNxpodService().server.getCostCenter(attributionId);
    ...
    const subscribeToStripe = useCallback(
        async (stripeParams: { paymentIntentId?: string; redirectStatus: string }) => {
            ...
            try {
                ...
                const newLimit = await getNxpodService().server.subscribeToStripe(
                    attributionId,
                    paymentIntentId || "",
                    limit,
                );
    ...
    const updateUsageLimit = useCallback(
        async (newLimit: number) => {
            ...
            try {
                await getNxpodService().server.setUsageLimit(attributionId, newLimit);
                setUsageLimit(newLimit);
